...
function _VirtualDom_makeCallback(eventNode, initialHandler)
{
	function callback(event)
	{
		...
		var currentEventNode = (
			...
			(tag == 2 ? value.b : tag == 3 && value.nN) && event.preventDefault(),
			eventNode
		);
		var tagger;
		var i;
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {s9: fragment, mS: host, uh: path, kj: port_, kq: protocol, uo: query};
	});
...
var $author$project$Pages$Splash$Model = F7(
	function (status, pagetitle, root, balances, primaryaddress, version, errors) {
		return {bH: balances, dF: errors, qj: pagetitle, f1: primaryaddress, qS: root, dL: status, bF: version};
	});
...
var $author$project$Pages$Connect$initialModel = {eZ: 0, e2: '', tl: false, m_: 'node.haveno.network:17750', f1: '', nQ: false, m4: false, vn: false};
...
var $elm$url$Url$Parser$State = F5(
	function (visited, unvisited, params, frag, value) {
		return {d$: frag, eb: params, dO: unvisited, ao: value, er: visited};
	});
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__BtcBalanceInfo = {
	...
	jo: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	kI: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	lS: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0)
};
...
var $eriktim$elm_protocol_buffers$Protobuf$Decode$stepMessage = F2(
	function (width, state) {
		return (state.dn <= 0) ? ($elm$core$Set$isEmpty(state.kF) ? $elm$bytes$Bytes$Decode$succeed(
						...
						function (_v3) {
							...
							return $elm$bytes$Bytes$Decode$Loop(
								_Utils_update(
									...
									{
										tT: fn(state.tT),
										kF: A2($elm$core$Set$remove, fieldNumber, state.kF),
		...
		return function (wireType) {
			if (wireType.$ === 2) {
				...
				return A2(
					...
					{nt: dict, tT: v, kF: requiredSet, dn: width},
...
var $eriktim$elm_protocol_buffers$Internal$Int64$xor = F2(
	function (_v0, _v1) {
		var a = _v0;
		var b = _v1;
		return A2($eriktim$elm_protocol_buffers$Internal$Int64$fromInts, a.tp ^ b.tp, a.nG ^ b.nG);
	});
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__XmrBalanceInfo = {
	...
	kf: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{r8: false, sl: r.sl, s1: r.s1, bI: r.bI, pW: r.pW, lO: r.lO, nk: r.nk, dl: r.dl}));
};
var $elm$http$Http$riskyRequest = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{r8: true, sl: r.sl, s1: r.s1, bI: r.bI, pW: r.pW, lO: r.lO, nk: r.nk, dl: r.dl}));
};
...
var $anmolitor$elm_grpc$Grpc$toCmd = F2(
	function (expect, _v0) {
		var req = _v0;
		var toHttpRequest = req.m5 ? $elm$http$Http$riskyRequest : $elm$http$Http$request;
		var _v1 = req.kM;
		var rpc = _v1;
		var body = A2(
		...
		return toHttpRequest(
			{
				...
				dl: _Utils_ap(
					req.mS,
					$anmolitor$elm_grpc$Grpc$rpcPath(req.kM))
			});
	});
				...
				case 0:
					...
					return A2(
						...
						$author$project$Pages$Splash$init(
							{pl: model.bF, lN: $elm$core$Maybe$Nothing}));
				case 1:
					var _v3 = _v0.a;
					return A2(
...
var $author$project$Main$init = F3(
	function (flag, _v0, key) {
		...
		var initialModel = {
			...
			qF: '',
			dL: 0,
			lN: $elm$time$Time$millisToPosix(0),
...
var $author$project$Main$JsMessage = F4(
	function (page, typeOfMsg, accountsData, currency) {
		return {g2: accountsData, oP: currency, H: page, nW: typeOfMsg};
	});
		...
		return $elm$json$Json$Encode$object(
			_List_fromArray(
				[
					...
					_Utils_Tuple2(
					'password',
					$elm$json$Json$Encode$string(model.ge))
				]));
	});
			...
			case 1:
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{sL: 0, f1: primaryAddresponse.um, dL: 0}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
			...
			case 2:
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{sL: 0, dL: 0, nc: subAddresponse.nc}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
			...
			case 0:
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{eZ: model.eZ, f1: primaryAddresponse.um, m4: false, vn: true}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{eZ: model.eZ + 1, m4: true}),
			...
			case 2:
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{sL: 1, dL: 0, nc: subAddresponse.nc}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
		...
		switch (msg.$) {
			...
			case 16:
				if (!msg.a.$) {
					...
					var updatedModel = _Utils_update(
						model,
						{dv: true, f1: primaryAddresponse.um, dL: 1});
					return A2(
						$author$project$Main$toAccounts,
						updatedModel,
						$author$project$Pages$Accounts$init(0));
						...
						switch (_v7) {
								...
								var accountsMdl = function () {
									...
									if (_v8.$ === 7) {
										...
										return _Utils_update(
											accountsModel,
											{
												sK: $author$project$Main$convertStringToCurrencyType(jsMsg.oP),
												tM: jsMsg.g2
											});
									} else {
							...
							case 'decryptedCryptoAccountsResponse':
								return A2(
									...
									A2(
										$author$project$Pages$Accounts$update,
										$author$project$Pages$Accounts$DecryptedBTCAddresses(jsMsg.g2),
	...
	return A2(
		...
		_List_fromArray(
			[
							...
							[
								...
								_List_fromArray(
									[
										...
										$elm$html$Html$Attributes$type_('text'),
										$elm$html$Html$Attributes$readonly(true),
										$elm$html$Html$Attributes$value('BTC: ' + model.jQ)
									]),
								_List_Nil)
							]))
					])),
				...
				$author$project$Pages$Accounts$AddNewCryptoAccount(model.jQ))
			]));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$elm$core$List$isEmpty(model.mV) ? _List_fromArray(
					[
						A2(
						$elm$html$Html$div,
						_List_fromArray(
...
var $author$project$Pages$Funds$formatBalance = function (int64) {
	...
	var fullUInt64 = function () {
		var lowPart = (int64.nG < 0) ? $MartinSStewart$elm_uint64$UInt64$fromInt(int64.nG + $author$project$Extras$Constants$xmrConversionConstant) : $MartinSStewart$elm_uint64$UInt64$fromInt(int64.nG);
		var highPart = A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								model.nF ? model.f1 : $author$project$Extras$Constants$blankAddress)
							]))
					]))
			]));
};
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						model.nF ? ($author$project$Pages$Funds$xmrAvailableBalanceAsString(model.bH) + ' XMR') : $author$project$Extras$Constants$blankAddress)
					]))
			]));
};
...
var $author$project$Utils$MyUtils$gotBalancesReplyAsTypeAlias = function (reply) {
	...
	var res1 = _v0.a;
	var _v1 = $eriktim$elm_protocol_buffers$Protobuf$Types$Int64$toInts(gotXmr.kf);
	var pend1 = _v1.a;
