...
{
	if (region.uX.ja === region.U.ja)
	{
		return 'on line ' + region.uX.ja;
	}
	return 'on lines ' + region.uX.ja + ' through ' + region.U.ja;
}
...
function _VirtualDom_makeCallback(eventNode, initialHandler)
{
	function callback(event)
	{
		...
		var stopPropagation = tag == 1 ? value.b : tag == 3 && value.nG;
		var currentEventNode = (
			stopPropagation && event.stopPropagation(),
			(tag == 2 ? value.b : tag == 3 && value.ny) && event.preventDefault(),
			eventNode
		);
		var tagger;
		var i;
	...
	return _Platform_initialize(
		...
		function(sendToApp, initialModel) {
			...
			return _Browser_makeAnimator(initialModel, function(model)
			{
				...
				var doc = view(model);
				var nextNode = _VirtualDom_node('body')(_List_Nil)(doc.sa);
				var patches = _VirtualDom_diff(currNode, nextNode);
				bodyNode = _VirtualDom_applyPatches(bodyNode, currNode, patches, sendToApp);
				currNode = nextNode;
				_VirtualDom_divertHrefToApp = 0;
				(title !== doc.vn) && (_VirtualDom_doc.title = title = doc.vn);
			});
		}
	);
});
...
{
	var onUrlChange = impl.t8;
	var onUrlRequest = impl.t9;
	...
	return _Browser_document({
		...
		tt: function(flags)
		{
			return A3(impl.tt, flags, _Browser_getUrl(), key);
		},
		vw: impl.vw,
		vu: impl.vu,
		u4: impl.u4
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { ti: 'hidden', sk: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { ti: 'mozHidden', sk: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { ti: 'msHidden', sk: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { ti: 'webkitHidden', sk: 'webkitvisibilitychange' }
		: { ti: 'hidden', sk: 'visibilitychange' };
}
...
{
	return {
		qQ: _Browser_getScene(),
		rA: {
			rJ: _Browser_window.pageXOffset,
			rL: _Browser_window.pageYOffset,
			dg: _Browser_doc.documentElement.clientWidth,
			is: _Browser_doc.documentElement.clientHeight
		}
	};
}
...
function _Browser_getScene()
{
	...
	return {
		dg: Math.max(body.scrollWidth, body.offsetWidth, elem.scrollWidth, elem.offsetWidth, elem.clientWidth),
		is: Math.max(body.scrollHeight, body.offsetHeight, elem.scrollHeight, elem.offsetHeight, elem.clientHeight)
	};
}
...
function _Browser_getViewportOf(id)
{
	return _Browser_withNode(id, function(node)
	{
		return {
			qQ: {
				dg: node.scrollWidth,
				is: node.scrollHeight
			},
			rA: {
				rJ: node.scrollLeft,
				rL: node.scrollTop,
				dg: node.clientWidth,
				is: node.clientHeight
			}
		};
	});
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		function done(response) {
			callback(toTask(request.sV.a(response)));
		}

		var xhr = new XMLHttpRequest();
		xhr.addEventListener('error', function() { done($elm$http$Http$NetworkError_); });
		xhr.addEventListener('timeout', function() { done($elm$http$Http$Timeout_); });
		xhr.addEventListener('load', function() { done(_Http_toResponse(request.sV.b, xhr)); });
		$elm$core$Maybe$isJust(request.vs) && _Http_track(router, xhr, request.vs.a);

		try {
			xhr.open(request.tQ, request.bF, true);
		} catch (e) {
		...
		request.sa.a && xhr.setRequestHeader('Content-Type', request.sa.a);
		xhr.send(request.sa.b);
...
{
	for (var headers = request.tg; headers.b; headers = headers.b) // WHILE_CONS
	{
		xhr.setRequestHeader(headers.a.a, headers.a.b);
	}
	xhr.timeout = request.vm.a || 0;
	xhr.responseType = request.sV.d;
	xhr.withCredentials = request.rZ;
}
...
function _Http_toMetadata(xhr)
{
	return {
		...
		uZ: xhr.status,
		u_: xhr.statusText,
		tg: _Http_parseHeaders(xhr.getAllResponseHeaders())
	};
}
...
function _Http_track(router, xhr, tracker)
{
	...
	xhr.addEventListener('progress', function(event) {
		...
		_Scheduler_rawSpawn(A2($elm$core$Platform$sendToSelf, router, _Utils_Tuple2(tracker, $elm$http$Http$Receiving({
			ur: event.loaded,
			uR: event.lengthComputable ? $elm$core$Maybe$Just(event.total) : $elm$core$Maybe$Nothing
		}))));
	});
}
		...
		while (true) {
			if (fromIndex < 0) {
				return A2(
					...
					false,
					{ar: nodeList, am: (len / $elm$core$Array$branchFactor) | 0, aq: tail});
			} else {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {s1: fragment, ni: host, uk: path, j4: port_, qu: protocol, uq: query};
	});
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__BalancesInfo = {m2: $elm$core$Maybe$Nothing, nQ: $elm$core$Maybe$Nothing};
...
var $author$project$Pages$Dashboard$initialModel = {
	m0: '0.00',
	oP: _List_Nil,
	s$: A6($elm$url$Url$Url, 0, 'localhost', $elm$core$Maybe$Nothing, '/dashboard', $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing),
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__BtcBalanceInfo = {
	ew: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	nn: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	nB: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	nJ: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0)
};
...
var $eriktim$elm_protocol_buffers$Internal$Int32$operations = {s2: $elm$core$Basics$identity, s3: $eriktim$elm_protocol_buffers$Internal$Int32$fromSigned, s4: $eriktim$elm_protocol_buffers$Internal$Int32$fromZigZag, um: $eriktim$elm_protocol_buffers$Internal$Int32$popBase128, up: $eriktim$elm_protocol_buffers$Internal$Int32$pushBase128, vo: $eriktim$elm_protocol_buffers$Internal$Int32$toSigned, vp: $eriktim$elm_protocol_buffers$Internal$Int32$toZigZag};
...
var $eriktim$elm_protocol_buffers$Protobuf$Decode$stepMessage = F2(
	function (width, state) {
		return (state.dg <= 0) ? ($elm$core$Set$isEmpty(state.kp) ? $elm$bytes$Bytes$Decode$succeed(
			$elm$bytes$Bytes$Decode$Done(
				_Utils_Tuple2(width, state.tU))) : $elm$bytes$Bytes$Decode$fail) : A2(
			$elm$bytes$Bytes$Decode$andThen,
			function (_v0) {
				var usedBytes = _v0.a;
				var _v1 = _v0.b;
						...
						function (_v3) {
							...
							return $elm$bytes$Bytes$Decode$Loop(
								_Utils_update(
									...
									{
										tU: fn(state.tU),
										kp: A2($elm$core$Set$remove, fieldNumber, state.kp),
										dg: (state.dg - usedBytes) - n
									}));
						},
						decoder(wireType));
				} else {
		...
		return function (wireType) {
			if (wireType.$ === 2) {
				...
				return A2(
					...
					{m6: dict, tU: v, kp: requiredSet, dg: width},
...
var $eriktim$elm_protocol_buffers$Internal$Int64$xor = F2(
	function (_v0, _v1) {
		var a = _v0;
		var b = _v1;
		return A2($eriktim$elm_protocol_buffers$Internal$Int64$fromInts, a.tj ^ b.tj, a.no ^ b.no);
	});
...
var $eriktim$elm_protocol_buffers$Internal$Int64$operations = {s2: $eriktim$elm_protocol_buffers$Internal$Int64$fromBase128, s3: $elm$core$Basics$identity, s4: $eriktim$elm_protocol_buffers$Internal$Int64$fromZigZag, um: $eriktim$elm_protocol_buffers$Internal$Int64$popBase128, up: $eriktim$elm_protocol_buffers$Internal$Int64$pushBase128, vo: $elm$core$Basics$identity, vp: $eriktim$elm_protocol_buffers$Internal$Int64$toZigZag};
...
var $eriktim$elm_protocol_buffers$Protobuf$Decode$uintDecoder = function (config) {
	return A2(
		...
		A2(
			$elm$bytes$Bytes$Decode$map,
			$elm$core$Tuple$mapSecond(config.s3),
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__XmrBalanceInfo = {
	ew: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	m0: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	nx: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	nC: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0),
	nD: A2($eriktim$elm_protocol_buffers$Protobuf$Types$Int64$fromInts, 0, 0)
};
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__GetBalancesReply = {oe: $elm$core$Maybe$Nothing};
...
var $eriktim$elm_protocol_buffers$Protobuf$Encode$toVarIntEncoders = F2(
	function (config, value) {
		var _v0 = config.um(value);
		var base128 = _v0.a;
		var higherBits = _v0.b;
		return _Utils_eq(
			higherBits,
			config.s2(0)) ? _List_fromArray(
			[
...
var $author$project$Proto$Io$Haveno$Protobuffer$Wallets$getBalances = {sF: $author$project$Proto$Io$Haveno$Protobuffer$decodeGetBalancesReply, sS: $author$project$Proto$Io$Haveno$Protobuffer$encodeGetBalancesRequest, uh: 'io.haveno.protobuffer', uF: 'GetBalances', uO: 'Wallets'};
...
var $anmolitor$elm_grpc$Grpc$new = F2(
	function (rpc, req) {
		return {
			...
			mK: false,
			ku: rpc,
			vm: $elm$core$Maybe$Nothing,
			vs: $elm$core$Maybe$Nothing
		};
	});
	...
	function (decoder, httpResponse) {
		var parseResponse = F3(
			function (isGoodStatus, metadata, bytes) {
				var defaultGrpcStatus = isGoodStatus ? 0 : $anmolitor$elm_grpc$Grpc$httpBadStatusToGrpcStatus(metadata.uZ);
				var grpcStatus = A2(
					...
					A2(
						...
						A2(
							...
							A2($elm$core$Dict$get, 'grpc-status', metadata.tg))));
				if (!grpcStatus) {
					return A2(
					...
					var errMessage = A2(
						$elm$core$Maybe$withDefault,
						metadata.u_,
						A2($elm$core$Dict$get, 'grpc-message', metadata.tg));
					return $elm$core$Result$Err(
						$anmolitor$elm_grpc$Grpc$BadStatus(
							{sT: errMessage, cu: metadata, ux: bytes, uY: grpcStatus}));
				}
			});
		switch (httpResponse.$) {
			case 4:
...
var $elm$http$Http$onEffects = F4(
	function (router, cmds, subs, state) {
		return A2(
			...
			A3($elm$http$Http$updateReqs, router, cmds, state.qC));
	});
			...
			return $elm$http$Http$Request(
				{
					rZ: r.rZ,
					sa: r.sa,
					sV: A2(_Http_mapExpect, func, r.sV),
					tg: r.tg,
					tQ: r.tQ,
					vm: r.vm,
					vs: r.vs,
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{rZ: false, sa: r.sa, sV: r.sV, tg: r.tg, tQ: r.tQ, vm: r.vm, vs: r.vs, bF: r.bF}));
};
var $elm$http$Http$riskyRequest = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{rZ: true, sa: r.sa, sV: r.sV, tg: r.tg, tQ: r.tQ, vm: r.vm, vs: r.vs, bF: r.bF}));
};
...
var $anmolitor$elm_grpc$Grpc$toCmd = F2(
	function (expect, _v0) {
		var req = _v0;
		var toHttpRequest = req.mK ? $elm$http$Http$riskyRequest : $elm$http$Http$request;
		var _v1 = req.ku;
		var rpc = _v1;
		var body = A2(
		...
		return toHttpRequest(
			{
				...
				sV: A2(
					...
					expect,
					$anmolitor$elm_grpc$Grpc$handleResponse(rpc.sF)),
				tg: req.tg,
				tQ: 'POST',
				vm: req.vm,
				vs: req.vs,
				bF: _Utils_ap(
					req.ni,
					$anmolitor$elm_grpc$Grpc$rpcPath(req.ku))
			});
	});
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__GetXmrPrimaryAddressReply = {uo: ''};
...
var $author$project$Proto$Io$Haveno$Protobuffer$Wallets$getXmrPrimaryAddress = {sF: $author$project$Proto$Io$Haveno$Protobuffer$decodeGetXmrPrimaryAddressReply, sS: $author$project$Proto$Io$Haveno$Protobuffer$encodeGetXmrPrimaryAddressRequest, uh: 'io.haveno.protobuffer', uF: 'GetXmrPrimaryAddress', uO: 'Wallets'};
...
var $author$project$Pages$Accounts$initialModel = {
	oe: $elm$core$Maybe$Just($author$project$Proto$Io$Haveno$Protobuffer$defaultBalancesInfo),
	sA: 0,
	oP: _List_Nil,
	mz: false,
	p8: 'Accounts',
...
var $author$project$Pages$Connect$init = function (_v0) {
	return _Utils_Tuple2(
		{su: 0, sD: '', te: false, tV: 'default.node.address:18081', uz: false, uA: false, vx: false},
		$elm$core$Platform$Cmd$none);
};
var $author$project$Pages$Dashboard$Model = F8(
	function (status, pagetitle, root, balance, flagUrl, havenoAPKHttpRequest, version, errors) {
		return {m0: balance, oP: errors, s$: flagUrl, nf: havenoAPKHttpRequest, p8: pagetitle, qH: root, uY: status, bd: version};
	});
...
var $author$project$Pages$Support$init = function (_v0) {
	return _Utils_Tuple2(
		_Utils_update(
			...
			{vn: 'Haveno-Web Support'}),
		$elm$core$Platform$Cmd$none);
};
var $elm$url$Url$Parser$State = F5(
	function (visited, unvisited, params, frag, value) {
		return {dS: frag, d1: params, dE: unvisited, al: value, eh: visited};
	});
		...
		return $elm$url$Url$Parser$getFirstMatch(
			parser(
				A5(
					...
					_List_Nil,
					$elm$url$Url$Parser$preparePath(url.uk),
					$elm$url$Url$Parser$prepareQuery(url.uq),
					url.s1,
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$AccountsPage(accounts)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$ConnectPage(connect)
				}),
...
var $author$project$Proto$Io$Haveno$Protobuffer$GetVersion$getVersion = {sF: $author$project$Proto$Io$Haveno$Protobuffer$decodeGetVersionReply, sS: $author$project$Proto$Io$Haveno$Protobuffer$encodeGetVersionRequest, uh: 'io.haveno.protobuffer', uF: 'GetVersion', uO: 'GetVersion'};
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$DashboardPage(dashboard)
				}),
			$elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$DonatePage(donate)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$FundsPage(funds)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$MarketPage(market)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$PortfolioPage(portfolio)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$BuyPage(pricing)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$SellPage(sell)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					H: $author$project$Main$SupportPage(support)
				}),
				...
				case 0:
					...
					return A2(
						...
						$author$project$Pages$Dashboard$init(
							{pb: model.bd, gr: $elm$core$Maybe$Nothing}));
				case 1:
					var _v3 = _v0.a;
					return A2(
...
var $author$project$Main$init = F3(
	function (flag, _v0, key) {
		...
		var updatedModel = {
			...
			oD: '',
			oP: _List_Nil,
			oZ: urlWithDashboardPath,
			nk: false,
			dx: false,
			dy: false,
			pw: key,
			H: $author$project$Main$DashboardPage($author$project$Pages$Dashboard$initialModel),
			mI: '',
			gr: $elm$time$Time$millisToPosix(0),
			bd: 'No Haveno version available',
			...
			case 1:
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{sA: 0, mI: primaryAddresponse.uo, uY: 1}),
						$elm$core$Platform$Cmd$none);
				} else {
					var error = msg.a.a;
					return _Utils_Tuple2(
			...
			case 2:
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{sA: 2, uY: 1, u2: subAddresponse.u2}),
						$elm$core$Platform$Cmd$none);
				} else {
					var error = msg.a.a;
					return _Utils_Tuple2(
		...
		switch (msg.$) {
			...
			case 4:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{te: true, uz: false, uA: false, vx: true}),
					$elm$core$Platform$Cmd$none);
			case 5:
				return _Utils_Tuple2(
					_Utils_update(
...
var $author$project$Pages$Dashboard$HavenoAPKHttpRequest = F6(
	function (method, headers, url, body, timeout, tracker) {
		return {sa: body, tg: headers, tQ: method, vm: timeout, vs: tracker, bF: url};
	});
		...
		if (!msg.$) {
			...
			if (!msg.a.$) {
				...
				var headers = _List_fromArray(
					[
						...
						'Bearer ' + A2(
							$elm$core$Maybe$withDefault,
							'No access token 2',
							$elm$core$Maybe$Just(auth.oF)))
					]);
				var flagUrlWithMongoDBMWAndPortUpdate = A2($elm$core$String$contains, $author$project$Extras$Constants$localorproductionServerAutoCheck, model.s$.ni) ? $elm$url$Url$toString(
					A6($elm$url$Url$Url, model.s$.qu, model.s$.ni, $elm$core$Maybe$Nothing, $author$project$Extras$Constants$middleWarePath, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing)) : $elm$url$Url$toString(
					A6(
						$elm$url$Url$Url,
						model.s$.qu,
						model.s$.ni,
				...
				var newModel = _Utils_update(
					model,
					{
						nf: $elm$core$Maybe$Just(newHttpParams)
					});
				return _Utils_Tuple2(newModel, $elm$core$Platform$Cmd$none);
			} else {
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__GetXmrNewSubaddressReply = {u2: ''};
...
var $author$project$Proto$Io$Haveno$Protobuffer$Wallets$getXmrNewSubaddress = {sF: $author$project$Proto$Io$Haveno$Protobuffer$decodeGetXmrNewSubaddressReply, sS: $author$project$Proto$Io$Haveno$Protobuffer$encodeGetXmrNewSubaddressRequest, uh: 'io.haveno.protobuffer', uF: 'GetXmrNewSubaddress', uO: 'Wallets'};
				...
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							...
							{
								dx: true,
								H: $author$project$Main$DashboardPage(newDashBoardModel),
								bd: verResp
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{dx: false, bd: 'Error obtaining version'}),
						$elm$core$Platform$Cmd$none);
				}
			case 15:
			...
			case 14:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							nS: $elm$core$Maybe$Just(newZone)
						}),
					$elm$core$Platform$Cmd$none);
			case 1:
			...
			[
				...
				_List_fromArray(
					[
						...
						((!$author$project$Main$isXMRWalletConnected(model)) && model.dx) ? 'status-dot green' : 'status-dot red')
					]),
				_List_Nil),
				$elm$html$Html$text(
				((!$author$project$Main$isXMRWalletConnected(model)) && model.dx) ? 'Connected' : ($author$project$Main$isXMRWalletConnected(model) ? 'Wallet not connected' : 'Haveno node not connected')),
				A2(
				$elm$html$Html$a,
				_List_fromArray(
					[
	...
	var links = A2(
		...
		_List_fromArray(
			[
				...
				navLink,
				2,
				{bD: 'Portfolio', bF: 'portfolio'}),
				A2(
				navLink,
				8,
				{bD: 'Accounts', bF: 'accounts'}),
				A2(
				navLink,
				9,
				{bD: 'Donate', bF: 'donate'})
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$Main$navLinks(model.H)
					]))
			]));
};
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$value(model.sD)
							]),
						_List_Nil),
						A2(
						$elm$html$Html$button,
...
var $mdgriffith$elm_ui$Internal$Model$asColumn = 1;
var $mdgriffith$elm_ui$Internal$Style$classes = {rN: 'a', nX: 'atv', rR: 'ab', rS: 'cx', rT: 'cy', rU: 'acb', rV: 'accx', rW: 'accy', rX: 'acr', n5: 'al', n6: 'ar', rY: 'at', mZ: 'ah', m_: 'av', r$: 's', r6: 'bh', r7: 'b', sb: 'w7', sd: 'bd', se: 'bdt', mg: 'bn', sf: 'bs', mk: 'cpe', so: 'cp', sp: 'cpx', sq: 'cpy', dv: 'c', mn: 'ctr', mo: 'cb', mp: 'ccx', dw: 'ccy', hH: 'cl', mq: 'cr', sx: 'ct', sB: 'cptr', sC: 'ctxt', s0: 'fcs', o$: 'focus-within', s5: 'fs', td: 'g', ne: 'hbh', ng: 'hc', pc: 'he', nh: 'hf', pd: 'hfp', tm: 'hv', tq: 'ic', ts: 'fr', my: 'lbl', tw: 'iml', tx: 'imlf', ty: 'imlp', tz: 'implw', tA: 'it', tG: 'i', pB: 'lnk', fz: 'nb', pY: 'notxt', t6: 'ol', t7: 'or', d0: 'oq', ug: 'oh', H: 'pg', p9: 'p', ui: 'ppe', qH: 'ui', qJ: 'r', uH: 'sb', uI: 'sbx', uJ: 'sby', uK: 'sbt', uQ: 'e', uS: 'cap', uT: 'sev', u1: 'sk', gq: 't', u8: 'tc', u9: 'w8', va: 'w2', vb: 'w9', vc: 'tj', mU: 'tja', vd: 'tl', ve: 'w3', vf: 'w5', vg: 'w4', vh: 'tr', vi: 'w6', vj: 'w1', vk: 'tun', rq: 'ts', ee: 'clr', vt: 'u', nM: 'wc', rH: 'we', nN: 'wf', rI: 'wfp', nP: 'wrp'};
var $mdgriffith$elm_ui$Internal$Model$Generic = {$: 0};
var $mdgriffith$elm_ui$Internal$Model$div = $mdgriffith$elm_ui$Internal$Model$Generic;
var $mdgriffith$elm_ui$Internal$Model$NoNearbyChildren = {$: 0};
var $mdgriffith$elm_ui$Internal$Model$columnClass = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dv);
var $mdgriffith$elm_ui$Internal$Model$gridClass = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.td);
var $mdgriffith$elm_ui$Internal$Model$pageClass = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.H);
var $mdgriffith$elm_ui$Internal$Model$paragraphClass = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.p9);
var $mdgriffith$elm_ui$Internal$Model$rowClass = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.qJ);
var $mdgriffith$elm_ui$Internal$Model$singleClass = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.uQ);
	...
	switch (style.$) {
		...
		case 8:
			...
			return 'grid-rows-' + (A2(
				$elm$core$String$join,
				'-',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$lengthClassName, template.uE)) + ('-cols-' + (A2(
				$elm$core$String$join,
				'-',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$lengthClassName, template.mm)) + ('-space-x-' + ($mdgriffith$elm_ui$Internal$Model$lengthClassName(template.uU.a) + ('-space-y-' + $mdgriffith$elm_ui$Internal$Model$lengthClassName(template.uU.b)))))));
		case 9:
			var pos = style.a;
			return 'gp grid-pos-' + ($elm$core$String$fromInt(pos.qJ) + ('-' + ($elm$core$String$fromInt(pos.ox) + ('-' + ($elm$core$String$fromInt(pos.dg) + ('-' + $elm$core$String$fromInt(pos.is)))))));
		case 11:
...
var $mdgriffith$elm_ui$Internal$Model$formatBoxShadow = function (shadow) {
	return A2(
		...
		A2(
			...
			_List_fromArray(
				[
					shadow.po ? $elm$core$Maybe$Just('inset') : $elm$core$Maybe$Nothing,
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.jK.a) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.jK.b) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.r9) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.uR) + 'px'),
					$elm$core$Maybe$Just(
					$mdgriffith$elm_ui$Internal$Model$formatColor(shadow.ss))
				])));
};
var $mdgriffith$elm_ui$Internal$Model$renderFocusStyle = function (focus) {
	return _List_fromArray(
		[
			...
			$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.o$) + ':focus-within',
			A2(
				$elm$core$List$filterMap,
				$elm$core$Basics$identity,
				_List_fromArray(
								...
								$mdgriffith$elm_ui$Internal$Model$formatBoxShadow(
									{
										...
										jK: A2(
											...
											A2($elm$core$Tuple$mapFirst, $elm$core$Basics$toFloat, shadow.jK)),
										uR: shadow.uR
									}));
						},
						focus.uP),
			...
			A2(
			$mdgriffith$elm_ui$Internal$Model$Style,
			($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + ':focus .focusable, ') + (($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + '.focusable:focus, ') + ('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + ' .focusable-thumb'))),
			A2(
				$elm$core$List$filterMap,
				$elm$core$Basics$identity,
				_List_fromArray(
...
var $mdgriffith$elm_ui$Internal$Style$selfName = function (desc) {
	switch (desc) {
		case 0:
			var _v1 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rY);
		case 1:
			var _v2 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rR);
		case 2:
			var _v3 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.n6);
		case 3:
			var _v4 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.n5);
		case 4:
			var _v5 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rS);
		default:
			var _v6 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rT);
	}
};
	...
	[
		...
		_Utils_ap(
			...
			_Utils_ap(
				...
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.tq))),
		_List_fromArray(
			[
		...
		A2(
		$mdgriffith$elm_ui$Internal$Style$Class,
		$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + ':focus',
		_List_fromArray(
			[
		...
		A2(
		$mdgriffith$elm_ui$Internal$Style$Class,
		$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.qH),
		_List_fromArray(
			[
				A2($mdgriffith$elm_ui$Internal$Style$Prop, 'width', '100%'),
				A2($mdgriffith$elm_ui$Internal$Style$Prop, 'height', 'auto'),
					...
					function (loc) {
						switch (loc) {
							case 0:
								return A2(
									$mdgriffith$elm_ui$Internal$Style$Descriptor,
									$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rN),
									_List_fromArray(
										[
		...
		_List_fromArray(
			[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.pY),
				_List_fromArray(
					[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sB),
				_List_fromArray(
					[
				...
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.tm, $mdgriffith$elm_ui$Internal$Style$classes.ee)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.tm, $mdgriffith$elm_ui$Internal$Style$classes.d0)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.s0, $mdgriffith$elm_ui$Internal$Style$classes.ee)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.s0, $mdgriffith$elm_ui$Internal$Style$classes.d0)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.nX, $mdgriffith$elm_ui$Internal$Style$classes.ee)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.nX, $mdgriffith$elm_ui$Internal$Style$classes.d0)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rq),
				_List_fromArray(
					[
						A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								A2(
								$mdgriffith$elm_ui$Internal$Style$Descriptor,
								$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.pB),
								_List_fromArray(
									[
										A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-basis', 'auto')
									]))
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rX,
						_List_fromArray(
							[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rV,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
								$mdgriffith$elm_ui$Internal$Style$Child,
								$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rS),
								_List_fromArray(
									[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rV,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:only-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rV,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
								$mdgriffith$elm_ui$Internal$Style$Child,
								$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rT),
								_List_fromArray(
									[
										A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin-top', 'auto !important'),
										A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin-bottom', 'auto !important')
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.rV + ' ~ u'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.rX + (' ~ s.' + $mdgriffith$elm_ui$Internal$Style$classes.rV)),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rU,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1')
							])),
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rW,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rW,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:only-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.rW,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.rW + ' ~ u'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.rU + (' ~ s.' + $mdgriffith$elm_ui$Internal$Style$classes.rW)),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$ + ':first-child'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
						...
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot(
							$mdgriffith$elm_ui$Internal$Style$classes.r$ + ($mdgriffith$elm_ui$Internal$Style$selfName(3) + (':first-child + .' + $mdgriffith$elm_ui$Internal$Style$classes.r$))),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
						...
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot(
							$mdgriffith$elm_ui$Internal$Style$classes.r$ + ($mdgriffith$elm_ui$Internal$Style$selfName(2) + (':first-child + .' + $mdgriffith$elm_ui$Internal$Style$classes.r$))),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.p9),
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'display', 'block'),
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.vg),
				_List_fromArray(
					[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.vf),
				_List_fromArray(
					[
...
var $mdgriffith$elm_ui$Internal$Style$explainer = '\n.explain {\n    border: 6px solid rgb(174, 121, 15) !important;\n}\n.explain > .' + ($mdgriffith$elm_ui$Internal$Style$classes.r$ + (' {\n    border: 4px dashed rgb(0, 151, 167) !important;\n}\n\n.ctr {\n    border: none !important;\n}\n.explain > .ctr > .' + ($mdgriffith$elm_ui$Internal$Style$classes.r$ + ' {\n    border: 4px dashed rgb(0, 151, 167) !important;\n}\n\n')));
...
var $mdgriffith$elm_ui$Internal$Style$overrides = '@media screen and (-ms-high-contrast: active), (-ms-high-contrast: none) {' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.qJ) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + (' { flex-basis: auto !important; } ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.qJ) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.mn) + (' { flex-basis: auto !important; }}' + ($mdgriffith$elm_ui$Internal$Style$inputTextReset + ($mdgriffith$elm_ui$Internal$Style$sliderReset + ($mdgriffith$elm_ui$Internal$Style$trackReset + ($mdgriffith$elm_ui$Internal$Style$thumbReset + $mdgriffith$elm_ui$Internal$Style$explainer)))))))))))))));
...
var $mdgriffith$elm_ui$Internal$Style$emptyIntermediate = F2(
	function (selector, closing) {
		return {ml: closing, ay: _List_Nil, dB: _List_Nil, cU: selector};
	});
					...
					case 3:
						...
						return _Utils_update(
							rendered,
							{
								ay: A2(
									$elm$core$List$cons,
									{ml: '\n}', ay: _List_Nil, dB: props, cU: '@supports (' + (prop + (':' + (value + (') {' + parent.cU))))},
									rendered.ay)
							});
					case 5:
						var selector = rule.a;
							...
							rendered,
							{
								ay: A2(
									...
									A2(
										...
										A2($mdgriffith$elm_ui$Internal$Style$emptyIntermediate, parent.cU + (' + ' + selector), ''),
										adjRules),
									rendered.ay)
							});
					case 1:
						...
						return _Utils_update(
							rendered,
							{
								ay: A2(
									...
									A2(
										$mdgriffith$elm_ui$Internal$Style$renderRules,
										A2($mdgriffith$elm_ui$Internal$Style$emptyIntermediate, parent.cU + (' > ' + child), ''),
										childRules),
									rendered.ay)
							});
					case 2:
								...
								ay: A2(
									...
									A2(
										...
										A2(
											...
											_Utils_ap(parent.cU, descriptor),
											''),
										descriptorRules),
									rendered.ay)
							});
						...
						return _Utils_update(
							rendered,
							{
								ay: A2(
									...
									A2(
										$mdgriffith$elm_ui$Internal$Style$renderRules,
										A2($mdgriffith$elm_ui$Internal$Style$emptyIntermediate, parent.cU, ''),
										batched),
									rendered.ay)
							});
				}
...
var $mdgriffith$elm_ui$Internal$Style$renderCompact = function (styleClasses) {
	...
	var renderClass = function (rule) {
		...
		if (!_v2.b) {
			...
			return rule.cU + ('{' + (renderValues(rule.dB) + (rule.ml + '}')));
		}
	};
	var renderIntermediate = function (_v0) {
		...
		return _Utils_ap(
			...
			$elm$core$String$concat(
				A2($elm$core$List$map, renderIntermediate, rule.ay)));
	};
	return $elm$core$String$concat(
		A2(
			$elm$core$List$map,
...
var $mdgriffith$elm_ui$Internal$Model$hasSmallCaps = function (typeface) {
	if (typeface.$ === 5) {
		var font = typeface.a;
		return A2($elm$core$List$any, $mdgriffith$elm_ui$Internal$Model$isSmallCaps, font.rw);
	} else {
		return false;
	}
};
				...
				case 0:
					...
					return _List_fromArray(
						[
							...
							('.' + ($mdgriffith$elm_ui$Internal$Style$classes.r$ + (':focus ' + (selector + '-fs  {')))) + (renderedProps + '\n}'),
							(selector + '-fs:focus-within {') + (renderedProps + '\n}'),
							('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.r$) + (' .focusable-thumb' + (selector + '-fs {')))) + (renderedProps + '\n}')
						]);
				default:
					return _List_fromArray(
						[
	...
	if (typeface.$ === 5) {
		...
		return $elm$core$Maybe$Just(
			A2(
				$elm$core$String$join,
				', ',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$renderVariant, font.rw)));
	} else {
		return $elm$core$Maybe$Nothing;
	}
};
	...
	function (options, rule, maybePseudo) {
		switch (rule.$) {
			...
			case 5:
				...
				var single = '.' + $mdgriffith$elm_ui$Internal$Style$classes.uQ;
				var row = '.' + $mdgriffith$elm_ui$Internal$Style$classes.qJ;
				var wrappedRow = '.' + ($mdgriffith$elm_ui$Internal$Style$classes.nP + row);
				var right = '.' + $mdgriffith$elm_ui$Internal$Style$classes.n6;
				var paragraph = '.' + $mdgriffith$elm_ui$Internal$Style$classes.p9;
				var page = '.' + $mdgriffith$elm_ui$Internal$Style$classes.H;
				var left = '.' + $mdgriffith$elm_ui$Internal$Style$classes.n5;
				var halfY = $elm$core$String$fromFloat(y / 2) + 'px';
				var halfX = $elm$core$String$fromFloat(x / 2) + 'px';
				var column = '.' + $mdgriffith$elm_ui$Internal$Style$classes.dv;
				var _class = '.' + cls;
				var any = '.' + $mdgriffith$elm_ui$Internal$Style$classes.r$;
				return $elm$core$List$concat(
					_List_fromArray(
						[
							A4(
			...
			case 8:
				...
				var xSpacing = toGridLength(template.uU.a);
				var ySpacing = toGridLength(template.uU.b);
				var rows = function (x) {
					...
					A2(
						$elm$core$String$join,
						' ',
						A2($elm$core$List$map, toGridLength, template.uE)));
				var msRows = function (x) {
					...
					A2(
						$elm$core$String$join,
						ySpacing,
						A2($elm$core$List$map, toGridLength, template.mm)));
				var msColumns = function (x) {
					...
					A2(
						...
						ySpacing,
						A2($elm$core$List$map, toGridLength, template.mm)));
				var gapY = 'grid-row-gap:' + (toGridLength(template.uU.b) + ';');
				var gapX = 'grid-column-gap:' + (toGridLength(template.uU.a) + ';');
			...
			case 9:
				...
				var msPosition = A2(
					...
					_List_fromArray(
						[
							'-ms-grid-row: ' + ($elm$core$String$fromInt(position.qJ) + ';'),
							'-ms-grid-row-span: ' + ($elm$core$String$fromInt(position.is) + ';'),
							'-ms-grid-column: ' + ($elm$core$String$fromInt(position.ox) + ';'),
							'-ms-grid-column-span: ' + ($elm$core$String$fromInt(position.dg) + ';')
						]));
				var modernPosition = A2(
					...
					_List_fromArray(
						[
							'grid-row: ' + ($elm$core$String$fromInt(position.qJ) + (' / ' + ($elm$core$String$fromInt(position.qJ + position.is) + ';'))),
							'grid-column: ' + ($elm$core$String$fromInt(position.ox) + (' / ' + ($elm$core$String$fromInt(position.ox + position.dg) + ';')))
						]));
				var _class = '.grid-pos-' + ($elm$core$String$fromInt(position.qJ) + ('-' + ($elm$core$String$fromInt(position.ox) + ('-' + ($elm$core$String$fromInt(position.dg) + ('-' + $elm$core$String$fromInt(position.is)))))));
...
var $mdgriffith$elm_ui$Internal$Model$renderFontAdjustmentRule = F3(
	function (fontToAdjust, _v0, otherFontName) {
		...
		return A2(
			...
			' ',
			_Utils_ap(
				A3($mdgriffith$elm_ui$Internal$Model$fontRule, name, $mdgriffith$elm_ui$Internal$Style$classes.uS, capital),
				A3($mdgriffith$elm_ui$Internal$Model$fontRule, name, $mdgriffith$elm_ui$Internal$Style$classes.s5, full)));
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					'.' + (name + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.uS + (', ' + ('.' + (name + (' .' + $mdgriffith$elm_ui$Internal$Style$classes.uS))))))),
					_List_fromArray(
						[
					...
					A2(
					$mdgriffith$elm_ui$Internal$Model$bracket,
					'.' + (name + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.uS + ('> .' + ($mdgriffith$elm_ui$Internal$Style$classes.gq + (', .' + (name + (' .' + ($mdgriffith$elm_ui$Internal$Style$classes.uS + (' > .' + $mdgriffith$elm_ui$Internal$Style$classes.gq)))))))))),
					_List_fromArray(
						[
...
var $mdgriffith$elm_ui$Internal$Model$adjust = F3(
	function (size, height, vertical) {
		return {is: height / size, uR: size, rz: vertical};
	});
...
var $mdgriffith$elm_ui$Internal$Model$convertAdjustment = function (adjustment) {
	var lines = _List_fromArray(
		[adjustment.si, adjustment.r5, adjustment.sJ, adjustment.tN]);
	var lineHeight = 1.5;
	...
	return {
		si: A3($mdgriffith$elm_ui$Internal$Model$adjust, capitalSize, ascender - newBaseline, capitalVertical),
		o0: A3($mdgriffith$elm_ui$Internal$Model$adjust, fullSize, ascender - descender, fullVertical)
	};
};
	...
	return _Utils_Tuple2(
		...
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'line-height',
				$elm$core$String$fromFloat(converted.is)),
				_Utils_Tuple2(
				'vertical-align',
				$elm$core$String$fromFloat(converted.rz) + 'em'),
				_Utils_Tuple2(
				'font-size',
				$elm$core$String$fromFloat(converted.uR) + 'em')
			]));
};
				...
				return {
					...
					lw: function () {
						...
						if (_v1.$ === 1) {
							...
							var topLevel = _v1.a;
							return A2($elm$core$List$cons, topLevel, rendered.lw);
						}
					}()
				};
			});
	...
	function (_static, opts, styles, children) {
		var dynamicStyleSheet = A2(
			...
			A3(
				...
				_Utils_Tuple2(
					$elm$core$Set$empty,
					$mdgriffith$elm_ui$Internal$Model$renderFocusStyle(opts.s0)),
				styles).b);
		return _static ? A2(
			$elm$core$List$cons,
			_Utils_Tuple2(
					...
					return A3(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.uQ))
									]))
							]));
			}
		}();
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.r$, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.mn, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.rX])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.r$, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.mn, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.rV])))
						]),
					_List_fromArray(
						[html])) : html));
			case 1:
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.r$, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.mn, $mdgriffith$elm_ui$Internal$Style$classes.rW])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.r$, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.mn, $mdgriffith$elm_ui$Internal$Style$classes.rU])))
						]),
					_List_fromArray(
						[html])) : html));
			default:
...
var $mdgriffith$elm_ui$Internal$Model$textElementClasses = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.gq + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.nM + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.ng)))));
...
var $mdgriffith$elm_ui$Internal$Model$textElementFillClasses = $mdgriffith$elm_ui$Internal$Style$classes.r$ + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.gq + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.nN + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.nh)))));
...
var $mdgriffith$elm_ui$Internal$Model$createElement = F3(
	function (context, children, rendered) {
					...
					case 1:
						...
						return _Utils_eq(context, $mdgriffith$elm_ui$Internal$Model$asParagraph) ? _Utils_Tuple2(
							A2(
								...
								_Utils_Tuple2(
									key,
									A2(styled.to, $mdgriffith$elm_ui$Internal$Model$NoStyleSheet, context)),
								htmls),
							$elm$core$List$isEmpty(existingStyles) ? styled.rc : _Utils_ap(styled.rc, existingStyles)) : _Utils_Tuple2(
							A2(
								$elm$core$List$cons,
								_Utils_Tuple2(
									key,
									A2(styled.to, $mdgriffith$elm_ui$Internal$Model$NoStyleSheet, context)),
								htmls),
							$elm$core$List$isEmpty(existingStyles) ? styled.rc : _Utils_ap(styled.rc, existingStyles));
					case 2:
						var str = child.a;
						return _Utils_Tuple2(
							A2(
		...
		if (children.$ === 1) {
			...
			var keyed = _v1.a;
			var styles = _v1.b;
			var newStyles = $elm$core$List$isEmpty(styles) ? rendered.rc : _Utils_ap(rendered.rc, styles);
			if (!newStyles.b) {
				return $mdgriffith$elm_ui$Internal$Model$Unstyled(
					A5(
						...
						$mdgriffith$elm_ui$Internal$Model$Keyed(
							A3($mdgriffith$elm_ui$Internal$Model$addKeyedChildren, 'nearby-element-pls', keyed, rendered.sl)),
				...
				return $mdgriffith$elm_ui$Internal$Model$Unstyled(
					A5(
						...
						$mdgriffith$elm_ui$Internal$Model$Unkeyed(
							A2($mdgriffith$elm_ui$Internal$Model$addChildren, unkeyed, rendered.sl)),
						$mdgriffith$elm_ui$Internal$Model$NoStyleSheet));
			} else {
				var allStyles = newStyles;
				return $mdgriffith$elm_ui$Internal$Model$Styled(
						...
						switch (location) {
							case 0:
								return A2(
									...
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fz, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.rN]));
							case 1:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fz, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.r7]));
							case 2:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fz, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.t7]));
							case 3:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fz, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.t6]));
							case 4:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fz, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.ts]));
							default:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fz, $mdgriffith$elm_ui$Internal$Style$classes.uQ, $mdgriffith$elm_ui$Internal$Style$classes.r6]));
						}
					}())
				]),
			_List_fromArray(
				[
					...
					switch (elem.$) {
						...
						default:
							var styled = elem.a;
							return A2(styled.to, $mdgriffith$elm_ui$Internal$Model$NoStyleSheet, $mdgriffith$elm_ui$Internal$Model$asEl);
					}
				}()
				]));
	});
...
var $mdgriffith$elm_ui$Internal$Model$alignXName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.mZ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.n5);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.mZ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.n6);
		default:
			return $mdgriffith$elm_ui$Internal$Style$classes.mZ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.rS);
	}
};
var $mdgriffith$elm_ui$Internal$Model$alignYName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.m_ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.rY);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.m_ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.rR);
		default:
			return $mdgriffith$elm_ui$Internal$Style$classes.m_ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.rT);
	}
};
	...
	switch (h.$) {
		case 0:
			...
			return _Utils_Tuple3(
				...
				$mdgriffith$elm_ui$Internal$Style$classes.pc + (' ' + name),
				_List_fromArray(
					[
		...
		case 2:
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				$mdgriffith$elm_ui$Internal$Style$classes.nh,
				_List_Nil) : _Utils_Tuple3(
				A2($mdgriffith$elm_ui$Internal$Flag$add, $mdgriffith$elm_ui$Internal$Flag$heightFill, $mdgriffith$elm_ui$Internal$Flag$none),
				$mdgriffith$elm_ui$Internal$Style$classes.pd + (' height-fill-' + $elm$core$String$fromInt(portion)),
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.r$ + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.dv + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
...
var $mdgriffith$elm_ui$Internal$Model$renderWidth = function (w) {
	switch (w.$) {
		case 0:
			...
			return _Utils_Tuple3(
				$mdgriffith$elm_ui$Internal$Flag$none,
				$mdgriffith$elm_ui$Internal$Style$classes.rH + (' width-px-' + $elm$core$String$fromInt(px)),
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
		...
		case 1:
			return _Utils_Tuple3(
				...
				$mdgriffith$elm_ui$Internal$Style$classes.nM,
				_List_Nil);
		case 2:
			var portion = w.a;
			return (portion === 1) ? _Utils_Tuple3(
				A2($mdgriffith$elm_ui$Internal$Flag$add, $mdgriffith$elm_ui$Internal$Flag$widthFill, $mdgriffith$elm_ui$Internal$Flag$none),
				$mdgriffith$elm_ui$Internal$Style$classes.nN,
				_List_Nil) : _Utils_Tuple3(
				A2($mdgriffith$elm_ui$Internal$Flag$add, $mdgriffith$elm_ui$Internal$Flag$widthFill, $mdgriffith$elm_ui$Internal$Flag$none),
				$mdgriffith$elm_ui$Internal$Style$classes.rI + (' width-fill-' + $elm$core$String$fromInt(portion)),
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.r$ + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.qJ + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
					...
					case 7:
						...
						if (A2($mdgriffith$elm_ui$Internal$Flag$present, $mdgriffith$elm_ui$Internal$Flag$width, has)) {
							...
							switch (width.$) {
								case 0:
									var px = width.a;
									var $temp$classes = ($mdgriffith$elm_ui$Internal$Style$classes.rH + (' width-px-' + $elm$core$String$fromInt(px))) + (' ' + classes),
										$temp$node = node,
									...
									continue gatherAttrRecursive;
								case 1:
									var $temp$classes = classes + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.nM),
										$temp$node = node,
										$temp$has = A2(
								...
								case 2:
									var portion = width.a;
									if (portion === 1) {
										var $temp$classes = classes + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.nN),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
										...
										var $temp$classes = classes + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.rI + (' width-fill-' + $elm$core$String$fromInt(portion)))),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
											$mdgriffith$elm_ui$Internal$Flag$widthFill,
						...
						if (A2($mdgriffith$elm_ui$Internal$Flag$present, $mdgriffith$elm_ui$Internal$Flag$height, has)) {
							...
							switch (height.$) {
								case 0:
									...
									var name = 'height-px-' + val;
									var $temp$classes = $mdgriffith$elm_ui$Internal$Style$classes.pc + (' ' + (name + (' ' + classes))),
										$temp$node = node,
								...
								case 2:
									...
									if (portion === 1) {
										var $temp$classes = $mdgriffith$elm_ui$Internal$Style$classes.nh + (' ' + classes),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
										...
										var $temp$classes = classes + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.pd + (' height-fill-' + $elm$core$String$fromInt(portion)))),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
											$mdgriffith$elm_ui$Internal$Flag$heightFill,
					...
					case 9:
						...
						var newStyles = function () {
							switch (elem.$) {
								...
								default:
									var styled = elem.a;
									return _Utils_ap(styles, styled.rc);
							}
						}();
						var $temp$classes = classes,
							$temp$node = node,
...
var $mdgriffith$elm_ui$Element$column = F2(
	function (attrs, children) {
		return A4(
			...
			A2(
				$elm$core$List$cons,
				$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.sx + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.hH)),
				A2(
					$elm$core$List$cons,
...
var $mdgriffith$elm_ui$Element$Font$bold = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$fontWeight, $mdgriffith$elm_ui$Internal$Style$classes.sb);
...
var $Orasund$elm_ui_framework$Framework$layoutOptions = $elm$core$List$singleton(
	$mdgriffith$elm_ui$Element$focusStyle(
		{
			r2: $elm$core$Maybe$Nothing,
			sc: $elm$core$Maybe$Just($Orasund$elm_ui_framework$Framework$Color$turquoise),
			uP: $elm$core$Maybe$Just(
				{
					r9: 10,
					ss: $Orasund$elm_ui_framework$Framework$Color$turquoise,
					jK: _Utils_Tuple2(0, 0),
					uR: 1
				})
		}));
...
var $mdgriffith$elm_ui$Internal$Model$focusDefaultStyle = {
	r2: $elm$core$Maybe$Nothing,
	sc: $elm$core$Maybe$Nothing,
	uP: $elm$core$Maybe$Just(
		{
			r9: 0,
			ss: A4($mdgriffith$elm_ui$Internal$Model$Rgba, 155 / 255, 203 / 255, 1, 1),
			jK: _Utils_Tuple2(0, 0),
			uR: 3
		})
};
var $mdgriffith$elm_ui$Internal$Model$optionsToRecord = function (options) {
				...
				case 0:
					...
					if (_v4.$ === 1) {
						return _Utils_update(
							record,
							{
								tm: $elm$core$Maybe$Just(hoverable)
							});
					} else {
						return record;
					}
				case 1:
					...
					if (_v5.$ === 1) {
						return _Utils_update(
							record,
							{
								s0: $elm$core$Maybe$Just(focusStyle)
							});
					} else {
						return record;
					}
				default:
					...
					if (_v6.$ === 1) {
						return _Utils_update(
							record,
							{
								tT: $elm$core$Maybe$Just(renderMode)
							});
					} else {
						return record;
					}
	...
	return andFinally(
		A3(
			$elm$core$List$foldr,
			combine,
			{s0: $elm$core$Maybe$Nothing, tm: $elm$core$Maybe$Nothing, tT: $elm$core$Maybe$Nothing},
			options));
};
			...
			A2(
				...
				$mdgriffith$elm_ui$Internal$Model$htmlClass(
					A2(
						...
						' ',
						_List_fromArray(
							[$mdgriffith$elm_ui$Internal$Style$classes.qH, $mdgriffith$elm_ui$Internal$Style$classes.r$, $mdgriffith$elm_ui$Internal$Style$classes.uQ]))),
...
var $Orasund$elm_ui_framework$Framework$layout = function (attributes) {
	return A2(
		...
		{uf: $Orasund$elm_ui_framework$Framework$layoutOptions},
		...
		A2(
			...
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$text('Your version is: ' + model.bd)),
					$mdgriffith$elm_ui$Element$text('\n'),
					function () {
					var _v0 = model.oP;
					if (!_v0.b) {
...
var $author$project$Pages$Funds$formatBalance = function (int64) {
	...
	var fullUInt64 = function () {
		var lowPart = (int64.no < 0) ? $MartinSStewart$elm_uint64$UInt64$fromInt(int64.no + $author$project$Extras$Constants$xmrConversionConstant) : $MartinSStewart$elm_uint64$UInt64$fromInt(int64.no);
		var highPart = A2(
			...
			$MartinSStewart$elm_uint64$UInt64$fromInt(int64.tj));
...
var $author$project$Pages$Funds$btcBalanceAsString = function (balInfo) {
	if (!balInfo.$) {
		...
		if (_v1.$ === 1) {
			...
			var _v2 = $eriktim$elm_protocol_buffers$Protobuf$Types$Int64$toInts(btcbalinfo.ew);
			var firstInt = _v2.a;
			var secondInt = _v2.b;
			return $author$project$Pages$Funds$formatBalance(
				{tj: firstInt, no: secondInt});
		}
	} else {
		return '';
	}
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								model.mz ? model.mI : $author$project$Extras$Constants$blankAddress)
							]))
					]))
			]));
};
var $author$project$Pages$Funds$reservedOfferBalanceAsString = function (balInfo) {
	if (!balInfo.$) {
		...
		if (_v1.$ === 1) {
			...
			var _v2 = $eriktim$elm_protocol_buffers$Protobuf$Types$Int64$toInts(xmrbalViewinfo.nC);
			var firstInt = _v2.a;
			var secondInt = _v2.b;
...
var $author$project$Pages$Funds$xmrAvailableBalanceAsString = function (balInfo) {
	if (!balInfo.$) {
		...
		if (_v1.$ === 1) {
			...
			var xmrbalViewinfo = _v1.a;
			var _v2 = $eriktim$elm_protocol_buffers$Protobuf$Types$Int64$toInts(xmrbalViewinfo.ew);
			var firstInt = _v2.a;
			var secondInt = _v2.b;
			return $author$project$Pages$Funds$formatBalance(
				{tj: firstInt, no: secondInt});
...
var $author$project$Pages$Funds$xmrBalView = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$text(
				'Available Balance: ' + ($author$project$Pages$Funds$xmrAvailableBalanceAsString(model.oe) + ' XMR'))
			]));
};
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						'Available BTC Balance: ' + ($author$project$Pages$Funds$btcBalanceAsString(model.oe) + ' BTC'))
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						'Reserved Offer Balance: ' + ($author$project$Pages$Funds$reservedOfferBalanceAsString(model.oe) + ' XMR'))
					])),
									...
									_List_fromArray(
										[
											...
											switch (_v1) {
												...
												case 2:
													return $author$project$Pages$Funds$subAddressView(model.u2);
												default:
		...
		switch (_v0.$) {
			...
			default:
				var connectModel = {
					...
					sD: '',
					te: model.dx,
					tV: 'default.node.address:18081',
					uz: false,
					uA: false,
					vx: $author$project$Main$isXMRWalletConnected(model)
				};
				return A2(
					$elm$html$Html$map,
...
var $author$project$Main$main = $elm$browser$Browser$application(
	{tt: $author$project$Main$init, t8: $author$project$Main$ChangedUrl, t9: $author$project$Main$ClickedLink, u4: $author$project$Main$subscriptions, vu: $author$project$Main$update, vw: $author$project$Main$view});
