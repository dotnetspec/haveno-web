...
{
	if (region.v$.jx === region.U.jx)
	{
		return 'on line ' + region.v$.jx;
	}
	return 'on lines ' + region.v$.jx + ' through ' + region.U.jx;
}
...
function _VirtualDom_makeCallback(eventNode, initialHandler)
{
	function callback(event)
	{
		...
		var value = result.a;
		var message = !tag ? value : tag < 3 ? value.a : value.uJ;
		var stopPropagation = tag == 1 ? value.b : tag == 3 && value.oa;
		var currentEventNode = (
			stopPropagation && event.stopPropagation(),
			(tag == 2 ? value.b : tag == 3 && value.n5) && event.preventDefault(),
			eventNode
		);
		var tagger;
		var i;
	...
	return _Platform_initialize(
		...
		function(sendToApp, initialModel) {
			...
			return _Browser_makeAnimator(initialModel, function(model)
			{
				...
				var doc = view(model);
				var nextNode = _VirtualDom_node('body')(_List_Nil)(doc.sT);
				var patches = _VirtualDom_diff(currNode, nextNode);
...
{
	var onUrlChange = impl.u2;
	var onUrlRequest = impl.u3;
	...
	return _Browser_document({
		...
		uk: function(flags)
		{
			return A3(impl.uk, flags, _Browser_getUrl(), key);
		},
		wA: impl.wA,
		wx: impl.wx,
		v6: impl.v6
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { t8: 'hidden', s3: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { t8: 'mozHidden', s3: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { t8: 'msHidden', s3: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { t8: 'webkitHidden', s3: 'webkitvisibilitychange' }
		: { t8: 'hidden', s3: 'visibilitychange' };
}
...
{
	return {
		rl: _Browser_getScene(),
		sb: {
			sl: _Browser_window.pageXOffset,
			sn: _Browser_window.pageYOffset,
...
{
	var flags = 'g';
	if (options.uQ) { flags += 'm'; }
	if (options.s0) { flags += 'i'; }
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		function done(response) {
			callback(toTask(request.m1.a(response)));
		}

		var xhr = new XMLHttpRequest();
		xhr.addEventListener('error', function() { done($elm$http$Http$NetworkError_); });
		xhr.addEventListener('timeout', function() { done($elm$http$Http$Timeout_); });
		xhr.addEventListener('load', function() { done(_Http_toResponse(request.m1.b, xhr)); });
		$elm$core$Maybe$isJust(request.wu) && _Http_track(router, xhr, request.wu.a);

		try {
			xhr.open(request.uK, request.bH, true);
		} catch (e) {
		...
		request.sT.a && xhr.setRequestHeader('Content-Type', request.sT.a);
		xhr.send(request.sT.b);
...
{
	for (var headers = request.t6; headers.b; headers = headers.b) // WHILE_CONS
	{
		xhr.setRequestHeader(headers.a.a, headers.a.b);
	}
	xhr.timeout = request.wo.a || 0;
	xhr.responseType = request.m1.d;
	xhr.withCredentials = request.sD;
}
...
function _Http_toMetadata(xhr)
{
	return {
		...
		v0: xhr.status,
		v1: xhr.statusText,
		t6: _Http_parseHeaders(xhr.getAllResponseHeaders())
	};
}
	...
	function (reverseNodeList, builder) {
		if (!builder.an) {
			return A4(
				...
				$elm$core$Elm$JsArray$length(builder.at),
			...
			return A4(
				$elm$core$Array$Array_elm_builtin,
				$elm$core$Elm$JsArray$length(builder.at) + treeLen,
		...
		while (true) {
			if (fromIndex < 0) {
				return A2(
					...
					false,
					{av: nodeList, an: (len / $elm$core$Array$branchFactor) | 0, at: tail});
			} else {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {tS: fragment, fr: host, f2: path, ks: port_, ng: protocol, vp: query};
	});
...
var $elm$regex$Regex$Match = F4(
	function (match, index, number, submatches) {
		return {iY: index, qc: match, W: number, v5: submatches};
	});
...
var $RomanErnst$erl$Erl$parse = function (str) {
	return {
		m2: $RomanErnst$erl$Erl$hasLeadingSlashFromAll(str),
		m3: $RomanErnst$erl$Erl$hasTrailingSlashFromAll(str),
		ae: $RomanErnst$erl$Erl$hashFromAll(str),
		fr: $RomanErnst$erl$Erl$host(str),
		n2: '',
		f2: $RomanErnst$erl$Erl$pathFromAll(str),
		ks: $RomanErnst$erl$Erl$extractPort(str),
		ng: $RomanErnst$erl$Erl$extractProtocol(str),
...
var $author$project$Pages$Dashboard$HavenoAPKHttpRequest = F6(
	function (method, headers, url, body, timeout, tracker) {
		return {sT: body, t6: headers, uK: method, wo: timeout, wu: tracker, bH: url};
	});
...
var $author$project$Pages$Dashboard$Model = F8(
	function (status, title, root, balance, flagUrl, havenoAPKHttpRequest, version, errors) {
		return {sM: balance, dJ: errors, fo: flagUrl, pH: havenoAPKHttpRequest, rc: root, rH: status, wp: title, ex: version};
	});
...
var $author$project$Proto$Io$Haveno$Protobuffer$Internals_$defaultProto__Io__Haveno__Protobuffer__GetVersionReply = {ex: ''};
...
var $eriktim$elm_protocol_buffers$Internal$Int32$operations = {tT: $elm$core$Basics$identity, tU: $eriktim$elm_protocol_buffers$Internal$Int32$fromSigned, tV: $eriktim$elm_protocol_buffers$Internal$Int32$fromZigZag, vi: $eriktim$elm_protocol_buffers$Internal$Int32$popBase128, vo: $eriktim$elm_protocol_buffers$Internal$Int32$pushBase128, wq: $eriktim$elm_protocol_buffers$Internal$Int32$toSigned, wr: $eriktim$elm_protocol_buffers$Internal$Int32$toZigZag};
...
var $eriktim$elm_protocol_buffers$Protobuf$Decode$stepMessage = F2(
	function (width, state) {
		return (state.$7 <= 0) ? ($elm$core$Set$isEmpty(state.kO) ? $elm$bytes$Bytes$Decode$succeed(
			$elm$bytes$Bytes$Decode$Done(
				_Utils_Tuple2(width, state.uO))) : $elm$bytes$Bytes$Decode$fail) : A2(
			$elm$bytes$Bytes$Decode$andThen,
			function (_v0) {
				var usedBytes = _v0.a;
				var _v1 = _v0.b;
				...
				var wireType = _v1.b;
				var _v2 = A2($elm$core$Dict$get, fieldNumber, state.nE);
				if (!_v2.$) {
					var decoder = _v2.a;
					return A2(
						$elm$bytes$Bytes$Decode$map,
						function (_v3) {
							...
							return $elm$bytes$Bytes$Decode$Loop(
								_Utils_update(
									...
									{
										uO: fn(state.uO),
										kO: A2($elm$core$Set$remove, fieldNumber, state.kO),
		...
		return function (wireType) {
			if (wireType.$ === 2) {
				...
				return A2(
					...
					{nE: dict, uO: v, kO: requiredSet, $7: width},
...
var $author$project$Proto$Io$Haveno$Protobuffer$GetVersion$getVersion = {tt: $author$project$Proto$Io$Haveno$Protobuffer$decodeGetVersionReply, tI: $author$project$Proto$Io$Haveno$Protobuffer$encodeGetVersionRequest, vb: 'io.haveno.protobuffer', vG: 'GetVersion', vR: 'GetVersion'};
				...
				var grpcStatus = A2(
					...
					A2(
						...
						A2(
							$elm$core$Maybe$andThen,
							$elm$core$String$toInt,
							A2($elm$core$Dict$get, 'grpc-status', metadata.t6))));
				if (!grpcStatus) {
					return A2(
						...
						A2(
							$elm$core$Maybe$andThen,
							function (response) {
								return A2($eriktim$elm_protocol_buffers$Protobuf$Decode$decode, decoder, response.uJ);
							},
					...
					var errMessage = A2(
						$elm$core$Maybe$withDefault,
						metadata.v1,
						A2($elm$core$Dict$get, 'grpc-message', metadata.t6));
					return $elm$core$Result$Err(
						$anmolitor$elm_grpc$Grpc$BadStatus(
							{tJ: errMessage, cB: metadata, vz: bytes, rH: grpcStatus}));
				}
			});
		switch (httpResponse.$) {
			case 4:
...
var $elm$http$Http$onEffects = F4(
	function (router, cmds, subs, state) {
		return A2(
			...
			A3($elm$http$Http$updateReqs, router, cmds, state.q7));
	});
			...
			return $elm$http$Http$Request(
				{
					sD: r.sD,
					sT: r.sT,
					m1: A2(_Http_mapExpect, func, r.m1),
					t6: r.t6,
					uK: r.uK,
					wo: r.wo,
					wu: r.wu,
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{sD: false, sT: r.sT, m1: r.m1, t6: r.t6, uK: r.uK, wo: r.wo, wu: r.wu, bH: r.bH}));
};
var $elm$http$Http$riskyRequest = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{sD: true, sT: r.sT, m1: r.m1, t6: r.t6, uK: r.uK, wo: r.wo, wu: r.wu, bH: r.bH}));
};
...
var $anmolitor$elm_grpc$Grpc$toCmd = F2(
	function (expect, _v0) {
		var req = _v0;
		var toHttpRequest = req.ni ? $elm$http$Http$riskyRequest : $elm$http$Http$request;
		var _v1 = req.kW;
		var rpc = _v1;
		var body = A2(
		...
		return toHttpRequest(
			{
				...
				m1: A2(
					...
					expect,
					$anmolitor$elm_grpc$Grpc$handleResponse(rpc.tt)),
				t6: req.t6,
				uK: 'POST',
				wo: req.wo,
				wu: req.wu,
								...
								return function (queryType) {
									return function (toMongoDBMWConfig) {
										return function (isValidNewAccessToken) {
											return function (isHardwareConnected) {
																					...
																					return function (searchterm) {
																						return function (searchResults) {
																							return function (objectJSONfromJSPort) {
																								return {au: apiSpecifics, sI: availableSlots, nB: datetimeFromMain, fi: emailpassword, dJ: errors, fo: flagUrl, ue: humanDateTime, nS: isHardwareConnected, nT: isReturnUser, d7: isValidNewAccessToken, m7: isWaitingForResponse, u_: objectJSONfromJSPort, vj: posixTmes, vq: queryType, vA: result, rc: root, vM: searchResults, rn: searchterm, n8: selectedSingleRank, aj: selectedranking, rH: status, wp: title, gM: toMongoDBMWConfig, aD: user};
																							};
																						};
																					};
																				};
...
var $author$project$Pages$Hardware$ApiSpecifics = F2(
	function (maxResults, accessToken) {
		return {dS: accessToken, uG: maxResults};
	});
...
var $author$project$Extras$Constants$emptyEmailPassword = {nI: '', n2: ''};
var $author$project$Data$Ranking$emptyRank = {
	ao: {z: 'String', dx: 'String'},
	aC: {z: 'String', dx: 'String'},
	bE: 0
};
																				...
																				return function (description) {
																					return function (credits) {
																						return function (addInfo) {
																							return {mN: active, sq: addInfo, su: age, tl: credits, ts: datestamp, dZ: description, nI: email, tH: emailValidationError, tY: gender, uv: isEmailInputFocused, uw: isMobileInputFocused, ux: isNameInputFocused, d9: memberRankings, nX: mobile, uM: mobileValidationError, uR: nameValidationError, dx: nickname, dL: ownedRankings, n2: password, vd: passwordValidationError, ws: token, wy: updatetext, r6: userid};
																						};
																					};
																				};
																			};
...
var $author$project$Data$User$emptyDescription = {nA: '', nU: ''};
...
var $AdrianRibao$elm_derberos_date$Derberos$Date$Core$DateRecord = F8(
	function (year, month, day, hour, minute, second, millis, zone) {
		return {m$: day, ua: hour, na: millis, nb: minute, nc: month, nm: second, oj: year, mL: zone};
	});
...
var $author$project$Pages$Hardware$init = function (fromMainToRankings) {
	var updatedFlagUrlToIncludeMongoDBMWSvr = A2($elm$core$String$contains, $author$project$Extras$Constants$localorproductionServerAutoCheck, fromMainToRankings.fo.fr) ? A6($elm$url$Url$Url, fromMainToRankings.fo.ng, fromMainToRankings.fo.fr, $elm$core$Maybe$Nothing, $author$project$Extras$Constants$productionProxyConfig, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing) : A6(
		$elm$url$Url$Url,
		fromMainToRankings.fo.ng,
		fromMainToRankings.fo.fr,
		$elm$core$Maybe$Just(3000),
		$author$project$Extras$Constants$middleWarePath,
		$elm$core$Maybe$Nothing,
		$elm$core$Maybe$Nothing);
	return _Utils_Tuple2(
		$author$project$Pages$Hardware$Model(1)('Hardware')(
			{fR: 'Loading...'})(fromMainToRankings.fo)(
			A2(
				$elm$core$Maybe$withDefault,
				$elm$core$Maybe$Nothing,
				$elm$core$Maybe$Just(fromMainToRankings.bL)))($author$project$Pages$Hardware$humandateTimePlaceholder)(_List_Nil)($author$project$Pages$Hardware$apiSpecsPlaceHolder)(
			...
			_List_fromArray(
				[$elm$core$Maybe$Nothing]))(false)(false)(
			{nI: '', n2: ''})($author$project$Data$Ranking$None)($author$project$Data$Ranking$emptyRank)($author$project$Data$User$emptySpectator)(2)('')(_List_Nil)($elm$core$Maybe$Nothing),
		$elm$core$Platform$Cmd$none);
};
...
var $elm$url$Url$Parser$State = F5(
	function (visited, unvisited, params, frag, value) {
		return {d4: frag, eh: params, dR: unvisited, am: value, ey: visited};
	});
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$DashboardPage(dashboard)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$FundsPage(funds)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$HardwarePage(hardware)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$MarketPage(market)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$PingPongPage(pingpong)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$PortfolioPage(portfolio)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$BuyPage(pricing)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$SellPage(sell)
				}),
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					D: $author$project$Main$SupportPage(support)
				}),
					...
					if (oauthCode.$ === 1) {
						return A2(
							...
							$author$project$Pages$Dashboard$init(
								{fo: model.dK, bL: $elm$core$Maybe$Nothing}));
					} else {
						if (oauthCode.a === '') {
							return A2(
								$author$project$Main$toDashboard,
	...
	function (flag, url, key) {
		...
		return A2(
			...
			url,
			{
				dJ: _List_fromArray(
					['']),
				dK: decodedJsonFromIndexjs,
				ju: navigate,
				D: $author$project$Main$NotFound,
				bL: $elm$time$Time$millisToPosix(0),
				mL: $elm$core$Maybe$Just($elm$time$Time$utc)
			});
	});
...
var $author$project$Extras$TestData$placeholderUrl = A6(
	$elm$url$Url$Url,
	0,
	'localhost',
	$elm$core$Maybe$Just(3000),
...
var $author$project$Main$subscriptions = function (_v0) {
	var _v1 = $author$project$Pages$Hardware$init(
		{fo: $author$project$Extras$TestData$placeholderUrl, bL: $elm$core$Maybe$Nothing});
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				$author$project$Pages$Hardware$hardwareSubscriptions(hardwaremodel))
			]));
};
				...
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								ex: $elm$core$Maybe$Just(versionResp)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
			...
			default:
				if (!msg.a.$) {
					...
					var headers = _List_fromArray(
						[
							...
							'Bearer ' + A2(
								$elm$core$Maybe$withDefault,
								'No access token 2',
								$elm$core$Maybe$Just(auth.o7)))
						]);
					var flagUrlWithMongoDBMWAndPortUpdate = A2($elm$core$String$contains, $author$project$Extras$Constants$localorproductionServerAutoCheck, model.fo.fr) ? $elm$url$Url$toString(
						A6($elm$url$Url$Url, model.fo.ng, model.fo.fr, $elm$core$Maybe$Nothing, $author$project$Extras$Constants$middleWarePath, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing)) : $elm$url$Url$toString(
						A6(
							$elm$url$Url$Url,
							model.fo.ng,
							model.fo.fr,
					...
					var newModel = _Utils_update(
						model,
						{
							pH: $elm$core$Maybe$Just(newHttpParams)
						});
...
var $author$project$Data$Ranking$Rank = F3(
	function (rank, player, challenger) {
		return {ao: challenger, aC: player, bE: rank};
	});
...
var $author$project$Data$Ranking$Ranking = F8(
	function (id, active, name, owner_id, baseaddress, ladder, player_count, owner_name) {
		return {mN: active, dG: baseaddress, z: id, m8: ladder, fR: name, f$: owner_id, n1: owner_name, n4: player_count};
	});
var $author$project$Data$Ranking$BaseAddress = F2(
	function (street, city) {
		return {h$: city, lF: street};
	});
...
var $author$project$Pages$Hardware$callRequest = function (model) {
	var headers = _List_fromArray(
		[
			...
			$elm$http$Http$header,
			'Authorization',
			'Bearer ' + A2($elm$core$Maybe$withDefault, 'No access token ', model.au.dS))
		]);
	var therequest = $elm$http$Http$request(
		{
			sT: $elm$http$Http$jsonBody($author$project$Extras$Constants$callRequestJson),
			m1: A2(
			...
			wo: $elm$core$Maybe$Nothing,
			wu: $elm$core$Maybe$Nothing,
			bH: 'https://ap-southeast-1.aws.realm.mongodb.com/api/client/v2.0/app/sr-espa1-snonq/functions/call'
		});
	return therequest;
};
	...
	function (user, rankingid) {
		if (user.$ === 1) {
			...
			return A2(
				$elm$core$List$filter,
				function (ranking) {
					return !_Utils_eq(ranking.z, rankingid);
				},
				usrInfo.d9);
		} else {
			return _List_Nil;
...
var $author$project$Data$Ranking$isCurrentlyInAChallenge = function (rank) {
	return (_Utils_eq(rank.aC.z, $author$project$Extras$Constants$noCurrentChallengerId) || _Utils_eq(rank.ao.z, $author$project$Extras$Constants$noCurrentChallengerId)) ? false : true;
};
var $author$project$Pages$Hardware$OperationEventMsg = function (operationEventMsg) {
	return {n$: operationEventMsg};
};
...
var $author$project$Pages$Hardware$SuccessfullProfileResult = F5(
	function (user_id, domain_id, identities, data, typeOfData) {
		return {to: data, tD: domain_id, uf: identities, r$: typeOfData, r5: user_id};
	});
var $author$project$Pages$Hardware$Identities = F4(
	function (id, provider_type, provider_id, provider_data) {
		return {z: id, vl: provider_data, vm: provider_id, vn: provider_type};
	});
...
var $author$project$Pages$Hardware$profileRequest = function (model) {
	...
	var therequest = $elm$http$Http$request(
		{
			sT: $elm$http$Http$emptyBody,
			m1: A2($elm$http$Http$expectJson, $author$project$Pages$Hardware$ProfileResponse, $author$project$Pages$Hardware$profileDecoder),
			t6: headers,
			uK: 'GET',
			wo: $elm$core$Maybe$Nothing,
			wu: $elm$core$Maybe$Nothing,
			bH: 'https://ap-southeast-1.aws.realm.mongodb.com/api/client/v2.0/auth/profile'
		});
	return therequest;
};
			...
			if (queryType.$ === 3) {
				...
				switch (msg.$) {
					...
					case 4:
						...
						return _Utils_update(
							userDetails,
							{n2: pword, vd: vldResult});
					case 6:
						var value = msg.a;
						var desc = userDetails.dZ;
		...
		switch (msg.$) {
			...
			case 49:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{vq: $author$project$Pages$Hardware$MemberSelectedView}),
					$elm$core$Platform$Cmd$none);
			case 59:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{vq: $author$project$Pages$Hardware$ConfirmDeleteUserView}),
					$elm$core$Platform$Cmd$none);
			case 60:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							vq: $author$project$Pages$Hardware$Login($author$project$Extras$Constants$emptyEmailPassword)
						}),
					$elm$core$Platform$Cmd$none);
			case 56:
				var newUser = $author$project$Data$User$gotUserInfo(model.aD);
				var newMemberRankings = function () {
					var _v1 = model.aj;
					if (_v1.$ === 1) {
						var ranking = _v1.a;
						return A2($author$project$Data$User$deleteRankingFromMemberRankings, model.aD, ranking.z);
					} else {
						return _List_fromArray(
				...
				var newUserInfo = _Utils_update(
					newUser,
					{d9: newMemberRankings});
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							vq: $author$project$Pages$Hardware$LoggedInUser,
							aD: $author$project$Data$User$Registered(newUserInfo)
						}),
					$elm$core$Platform$Cmd$none);
			case 54:
				return _Utils_Tuple2(
			...
			case 58:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{vq: $author$project$Pages$Hardware$ConfirmLeaveMemberView}),
					$elm$core$Platform$Cmd$none);
			case 55:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{vq: $author$project$Pages$Hardware$SpectatorSelectedView}),
					$elm$core$Platform$Cmd$none);
			case 53:
				var _v2 = model.aD;
				if (_v2.$ === 1) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{vq: $author$project$Pages$Hardware$ConfirmJoinMemberView}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								vq: $author$project$Pages$Hardware$RegisterUser($author$project$Data$User$emptyUserInfo)
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 33:
			...
			case 44:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{vq: $author$project$Pages$Hardware$ConfirmDeleteOwnedRanking}),
					$elm$core$Platform$Cmd$none);
			case 40:
				var value = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							aj: $author$project$Data$Ranking$Owned(
								A2(
									$author$project$Data$Ranking$updatedRankingName,
									$author$project$Data$Ranking$gotRanking(model.aj),
									value))
						}),
					$elm$core$Platform$Cmd$none);
			case 41:
			...
			case 27:
				...
				var newRank = A3(
					...
					1,
					{z: userInfo.r6, dx: userInfo.dx},
					{z: $author$project$Extras$Constants$noCurrentChallengerId, dx: 'Challenger'});
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							vq: $author$project$Pages$Hardware$CreatingNewLadder(userInfo),
							aj: $author$project$Data$Ranking$Owned(
								_Utils_update(
									newRanking,
									{
			...
			case 24:
				...
				var decodedConnectLNSPublicKey = function () {
				...
				var updatedIsConnected = (decodedConnectLNSPublicKey === '1KrEBrdLTotPZWDRQN1WUn7PDbXA7fwfsS') ? true : false;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{nS: updatedIsConnected}),
					$elm$core$Platform$Cmd$none);
			case 34:
				return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			case 43:
					...
					function () {
						...
						switch (_v5.$) {
							case 9:
								return _Utils_update(
									model,
									{vq: $author$project$Pages$Hardware$OwnedSelectedView});
							case 11:
								return _Utils_update(
									model,
									{vq: $author$project$Pages$Hardware$MemberSelectedView});
			...
			case 16:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{nB: dateTimeOnlyUpdatedIfNotSelected}),
					$elm$core$Platform$Cmd$none);
			case 15:
				return _Utils_Tuple2(
					_Utils_update(
				...
				if (!msg.a.$) {
					...
					var newModel = _Utils_update(
						model,
						{
							d7: true,
							vq: $author$project$Pages$Hardware$LoggedInUser,
							gM: $elm$core$Maybe$Just(newHttpParams),
							aD: $author$project$Data$User$Registered(auth)
						});
					var apiSpecs = model.au;
					return _Utils_Tuple2(newModel, $elm$core$Platform$Cmd$none);
				} else {
					var responseErr = msg.a.a;
					var respErr = $author$project$Extras$Constants$httpErrorToString(responseErr);
					...
					var newModel = _Utils_update(
						model,
						{
							au: _Utils_update(
								apiSpecs,
								{
									dS: $elm$core$Maybe$Just(auth.nw)
								}),
							d7: true,
							vq: $author$project$Pages$Hardware$LoggedInUser,
							gM: $elm$core$Maybe$Just(newHttpParams)
				...
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							...
							{
								m7: false,
								aj: $author$project$Data$Ranking$Spectator(specRankingResult)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var responseErr = msg.a.a;
			...
			case 51:
				var rankingId = msg.a;
				var updatedFlagUrlToIncludeMongoDBMWSvr = A2($elm$core$String$contains, $author$project$Extras$Constants$localorproductionServerAutoCheck, model.fo.fr) ? $elm$url$Url$toString(
					A6($elm$url$Url$Url, model.fo.ng, model.fo.fr, $elm$core$Maybe$Nothing, $author$project$Extras$Constants$productionProxyConfig, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing)) : $elm$url$Url$toString(
					A6(
						$elm$url$Url$Url,
						model.fo.ng,
						model.fo.fr,
				...
				var updatModelWithNewPostData = _Utils_update(
					model,
					{
						...
						m7: true,
						vq: $author$project$Pages$Hardware$SpectatorSelectedView,
						aj: $author$project$Data$Ranking$Spectator($author$project$Data$Ranking$emptyRanking),
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'message',
				$elm$json$Json$Encode$string(request.uJ))
			]));
};
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 0:
				var therequest = $elm$http$Http$request(
					{
						sT: $elm$http$Http$jsonBody(
							$author$project$Pages$PingPong$encodePingRequest(
								{uJ: 'Ping'})),
						m1: A2($elm$http$Http$expectJson, $author$project$Pages$PingPong$Receive, $author$project$Pages$PingPong$decodePongResponse),
						t6: _List_Nil,
						uK: 'POST',
						wo: $elm$core$Maybe$Nothing,
						wu: $elm$core$Maybe$Nothing,
						bH: 'http://localhost:9001/ping'
					});
				return _Utils_Tuple2(
					_Utils_update(
						model,
				...
				if (!msg.a.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								rc: {fR: 'Error receiving response'}
							}),
						$elm$core$Platform$Cmd$none);
				}
		}
			...
			case 12:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							mL: $elm$core$Maybe$Just(newZone)
						}),
					$elm$core$Platform$Cmd$none);
			case 0:
					...
					if (hardwareMsg.$ === 23) {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									D: $author$project$Main$HardwarePage(newHardwareModel)
								}),
						...
						return A2(
							$author$project$Main$toHardware,
							model,
							A2($author$project$Pages$Hardware$update, hardwareMsg, hardwareModel));
					}
				} else {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				}
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Version 0.0.4')
								]))
						]))
				]))
		]));
	...
	var links = A2(
		...
		_List_fromArray(
			[
				...
				navLink,
				4,
				{bl: 'Support', bH: 'support'}),
				A2(
				navLink,
				5,
				{bl: 'PingPong', bH: 'pingpong'}),
				A2(
				navLink,
				1,
				{bl: 'Sell', bH: 'sell'}),
				...
				navLink,
				8,
				{bl: 'Hardware', bH: 'hardware'}),
				A2(
				navLink,
				2,
				{bl: 'Portfolio', bH: 'portfolio'})
			]));
	return links;
};
	...
	var links = A2(
		...
		_List_fromArray(
			[
				...
				navLink(
				{bl: 'other potential support', bH: 'support'})
			]));
	return links;
}();
...
var $mdgriffith$elm_ui$Internal$Model$asColumn = 1;
var $mdgriffith$elm_ui$Internal$Style$classes = {sp: 'a', mN: 'atv', sv: 'ab', sw: 'cx', sx: 'cy', sy: 'acb', sz: 'accx', sA: 'accy', sB: 'acr', ox: 'al', oy: 'ar', sC: 'at', nx: 'ah', ny: 'av', sF: 's', sP: 'bh', sQ: 'b', sU: 'w7', sW: 'bd', sX: 'bdt', mP: 'bn', sY: 'bs', mT: 'cpe', s7: 'cp', s8: 'cpx', s9: 'cpy', dH: 'c', mW: 'ctr', mX: 'cb', mY: 'ccx', dI: 'ccy', h3: 'cl', mZ: 'cr', tj: 'ct', tm: 'cptr', tn: 'ctxt', tR: 'fcs', ps: 'focus-within', tW: 'fs', t4: 'g', nN: 'hbh', nO: 'hc', pI: 'he', nP: 'hf', pJ: 'hfp', ub: 'hv', uh: 'ic', uj: 'fr', m6: 'lbl', un: 'iml', uo: 'imlf', up: 'imlp', uq: 'implw', ur: 'it', uy: 'i', p8: 'lnk', fT: 'nb', qu: 'notxt', u0: 'ol', u1: 'or', eg: 'oq', u9: 'oh', D: 'pg', qF: 'p', vc: 'ppe', rc: 'ui', re: 'r', vI: 'sb', vJ: 'sbx', vK: 'sby', vL: 'sbt', vU: 'e', vW: 'cap', vX: 'sev', v4: 'sk', dE: 't', wa: 'tc', wb: 'w8', wc: 'w2', wd: 'w9', we: 'tj', nr: 'tja', wf: 'tl', wg: 'w3', wh: 'w5', wi: 'w4', wj: 'tr', wk: 'w6', wl: 'w1', wm: 'tun', rZ: 'ts', eu: 'clr', ww: 'u', og: 'wc', sj: 'we', oh: 'wf', sk: 'wfp', oi: 'wrp'};
var $mdgriffith$elm_ui$Internal$Model$Generic = {$: 0};
var $mdgriffith$elm_ui$Internal$Model$div = $mdgriffith$elm_ui$Internal$Model$Generic;
var $mdgriffith$elm_ui$Internal$Model$NoNearbyChildren = {$: 0};
var $mdgriffith$elm_ui$Internal$Model$columnClass = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dH);
var $mdgriffith$elm_ui$Internal$Model$gridClass = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.t4);
var $mdgriffith$elm_ui$Internal$Model$pageClass = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.D);
var $mdgriffith$elm_ui$Internal$Model$paragraphClass = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.qF);
var $mdgriffith$elm_ui$Internal$Model$rowClass = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.re);
var $mdgriffith$elm_ui$Internal$Model$singleClass = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.vU);
	...
	switch (style.$) {
		...
		case 8:
			...
			return 'grid-rows-' + (A2(
				...
				$elm$core$String$join,
				'-',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$lengthClassName, template.mV)) + ('-space-x-' + ($mdgriffith$elm_ui$Internal$Model$lengthClassName(template.vY.a) + ('-space-y-' + $mdgriffith$elm_ui$Internal$Model$lengthClassName(template.vY.b)))))));
		case 9:
			var pos = style.a;
			return 'gp grid-pos-' + ($elm$core$String$fromInt(pos.re) + ('-' + ($elm$core$String$fromInt(pos.o_) + ('-' + ($elm$core$String$fromInt(pos.$7) + ('-' + $elm$core$String$fromInt(pos.iP)))))));
		case 11:
...
var $mdgriffith$elm_ui$Internal$Model$formatBoxShadow = function (shadow) {
	return A2(
		...
		A2(
			...
			_List_fromArray(
				[
					shadow.pT ? $elm$core$Maybe$Just('inset') : $elm$core$Maybe$Nothing,
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.j5.a) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.j5.b) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.sS) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.vV) + 'px'),
					$elm$core$Maybe$Just(
					$mdgriffith$elm_ui$Internal$Model$formatColor(shadow.tb))
				])));
};
var $mdgriffith$elm_ui$Internal$Model$renderFocusStyle = function (focus) {
	return _List_fromArray(
		[
			...
			$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ps) + ':focus-within',
			A2(
				$elm$core$List$filterMap,
				$elm$core$Basics$identity,
				_List_fromArray(
								...
								$mdgriffith$elm_ui$Internal$Model$formatBoxShadow(
									{
										...
										j5: A2(
											...
											A2($elm$core$Tuple$mapFirst, $elm$core$Basics$toFloat, shadow.j5)),
										vV: shadow.vV
									}));
						},
						focus.vS),
			...
			A2(
			$mdgriffith$elm_ui$Internal$Model$Style,
			($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + ':focus .focusable, ') + (($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + '.focusable:focus, ') + ('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + ' .focusable-thumb'))),
			A2(
				$elm$core$List$filterMap,
				$elm$core$Basics$identity,
				_List_fromArray(
...
var $mdgriffith$elm_ui$Internal$Style$contentName = function (desc) {
	switch (desc) {
		case 0:
			var _v1 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.tj);
		case 1:
			var _v2 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.mX);
		case 2:
			var _v3 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.mZ);
		case 3:
			var _v4 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.h3);
		case 4:
			var _v5 = desc;
...
var $mdgriffith$elm_ui$Internal$Style$selfName = function (desc) {
	switch (desc) {
		...
		case 1:
			var _v2 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sv);
		case 2:
			var _v3 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.oy);
		case 3:
			var _v4 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ox);
		case 4:
			var _v5 = desc;
			return $mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sw);
		default:
			var _v6 = desc;
	...
	var createDescription = function (alignment) {
		...
		return _List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF),
				_List_fromArray(
					[
						A2(
						$mdgriffith$elm_ui$Internal$Style$Descriptor,
	...
	[
		...
		_List_fromArray(
			[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Child,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sP),
				_List_fromArray(
					[
		...
		A2(
		$mdgriffith$elm_ui$Internal$Style$Child,
		$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.nO),
		_List_fromArray(
			[
...
var $mdgriffith$elm_ui$Internal$Style$baseSheet = _List_fromArray(
	[
		...
		A2(
		$mdgriffith$elm_ui$Internal$Style$Class,
		$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + ':focus',
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Child,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.uj),
				_List_fromArray(
					[
						A2(
						$mdgriffith$elm_ui$Internal$Style$Descriptor,
					...
					function (loc) {
						switch (loc) {
							...
							case 3:
								return A2(
									$mdgriffith$elm_ui$Internal$Style$Descriptor,
									$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.u0),
									_List_fromArray(
										[
											A2($mdgriffith$elm_ui$Internal$Style$Prop, 'position', 'absolute'),
		...
		_List_fromArray(
			[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.qu),
				_List_fromArray(
					[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.tm),
				_List_fromArray(
					[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.tn),
				_List_fromArray(
					[
				...
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.ub, $mdgriffith$elm_ui$Internal$Style$classes.eu)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.ub, $mdgriffith$elm_ui$Internal$Style$classes.eg)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.tR, $mdgriffith$elm_ui$Internal$Style$classes.eu)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.tR, $mdgriffith$elm_ui$Internal$Style$classes.eg)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.mN, $mdgriffith$elm_ui$Internal$Style$classes.eu)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.mN, $mdgriffith$elm_ui$Internal$Style$classes.eg)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.rZ),
				_List_fromArray(
					[
						A2(
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Descriptor,
						$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.re),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-shrink', '1')
							]))
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.s8),
				_List_fromArray(
					[
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sX),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.pJ),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'align-self', 'stretch !important')
							])),
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sB,
						_List_fromArray(
							[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sz,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sz,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:only-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sz,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.sz + ' ~ u'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.sB + (' ~ s.' + $mdgriffith$elm_ui$Internal$Style$classes.sz)),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Descriptor,
						$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.m6),
						_List_fromArray(
							[
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sy,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1')
							])),
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sA,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sA,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:only-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.sA,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.sA + ' ~ u'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.sy + (' ~ s.' + $mdgriffith$elm_ui$Internal$Style$classes.sA)),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF + ':first-child'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
						...
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot(
							$mdgriffith$elm_ui$Internal$Style$classes.sF + ($mdgriffith$elm_ui$Internal$Style$selfName(3) + (':first-child + .' + $mdgriffith$elm_ui$Internal$Style$classes.sF))),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
						...
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot(
							$mdgriffith$elm_ui$Internal$Style$classes.sF + ($mdgriffith$elm_ui$Internal$Style$selfName(2) + (':first-child + .' + $mdgriffith$elm_ui$Internal$Style$classes.sF))),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.qF),
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'display', 'block'),
				...
				A2(
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sU),
				_List_fromArray(
					[
...
var $mdgriffith$elm_ui$Internal$Style$explainer = '\n.explain {\n    border: 6px solid rgb(174, 121, 15) !important;\n}\n.explain > .' + ($mdgriffith$elm_ui$Internal$Style$classes.sF + (' {\n    border: 4px dashed rgb(0, 151, 167) !important;\n}\n\n.ctr {\n    border: none !important;\n}\n.explain > .ctr > .' + ($mdgriffith$elm_ui$Internal$Style$classes.sF + ' {\n    border: 4px dashed rgb(0, 151, 167) !important;\n}\n\n')));
...
var $mdgriffith$elm_ui$Internal$Style$overrides = '@media screen and (-ms-high-contrast: active), (-ms-high-contrast: none) {' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.re) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + (' { flex-basis: auto !important; } ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.re) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.mW) + (' { flex-basis: auto !important; }}' + ($mdgriffith$elm_ui$Internal$Style$inputTextReset + ($mdgriffith$elm_ui$Internal$Style$sliderReset + ($mdgriffith$elm_ui$Internal$Style$trackReset + ($mdgriffith$elm_ui$Internal$Style$thumbReset + $mdgriffith$elm_ui$Internal$Style$explainer)))))))))))))));
...
var $mdgriffith$elm_ui$Internal$Style$emptyIntermediate = F2(
	function (selector, closing) {
		return {mU: closing, aB: _List_Nil, dO: _List_Nil, c0: selector};
	});
					...
					case 3:
						...
						return _Utils_update(
							rendered,
							{
								aB: A2(
									$elm$core$List$cons,
									{mU: '\n}', aB: _List_Nil, dO: props, c0: '@supports (' + (prop + (':' + (value + (') {' + parent.c0))))},
									rendered.aB)
							});
					case 5:
						var selector = rule.a;
							...
							rendered,
							{
								aB: A2(
									...
									A2(
										...
										A2($mdgriffith$elm_ui$Internal$Style$emptyIntermediate, parent.c0 + (' + ' + selector), ''),
										adjRules),
									rendered.aB)
							});
					case 1:
						...
						return _Utils_update(
							rendered,
							{
								aB: A2(
									...
									A2(
										$mdgriffith$elm_ui$Internal$Style$renderRules,
										A2($mdgriffith$elm_ui$Internal$Style$emptyIntermediate, parent.c0 + (' > ' + child), ''),
										childRules),
									rendered.aB)
							});
					case 2:
								...
								aB: A2(
									...
									A2(
										...
										A2(
											...
											_Utils_ap(parent.c0, descriptor),
											''),
										descriptorRules),
									rendered.aB)
							});
						...
						return _Utils_update(
							rendered,
							{
								aB: A2(
									...
									A2(
										$mdgriffith$elm_ui$Internal$Style$renderRules,
										A2($mdgriffith$elm_ui$Internal$Style$emptyIntermediate, parent.c0, ''),
										batched),
									rendered.aB)
							});
				}
...
var $mdgriffith$elm_ui$Internal$Style$renderCompact = function (styleClasses) {
	...
	var renderClass = function (rule) {
		...
		if (!_v2.b) {
			...
			return rule.c0 + ('{' + (renderValues(rule.dO) + (rule.mU + '}')));
		}
	};
...
var $mdgriffith$elm_ui$Internal$Model$hasSmallCaps = function (typeface) {
	if (typeface.$ === 5) {
		var font = typeface.a;
		return A2($elm$core$List$any, $mdgriffith$elm_ui$Internal$Model$isSmallCaps, font.r7);
	} else {
		return false;
	}
};
				...
				case 0:
					...
					return _List_fromArray(
						[
							...
							('.' + ($mdgriffith$elm_ui$Internal$Style$classes.sF + (':focus ' + (selector + '-fs  {')))) + (renderedProps + '\n}'),
							(selector + '-fs:focus-within {') + (renderedProps + '\n}'),
							('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.sF) + (' .focusable-thumb' + (selector + '-fs {')))) + (renderedProps + '\n}')
						]);
				default:
					return _List_fromArray(
						[
	...
	if (typeface.$ === 5) {
		...
		return $elm$core$Maybe$Just(
			A2(
				$elm$core$String$join,
				', ',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$renderVariant, font.r7)));
	} else {
		return $elm$core$Maybe$Nothing;
	}
};
	...
	function (options, rule, maybePseudo) {
		switch (rule.$) {
			...
			case 5:
				...
				var row = '.' + $mdgriffith$elm_ui$Internal$Style$classes.re;
				var wrappedRow = '.' + ($mdgriffith$elm_ui$Internal$Style$classes.oi + row);
				var right = '.' + $mdgriffith$elm_ui$Internal$Style$classes.oy;
				var paragraph = '.' + $mdgriffith$elm_ui$Internal$Style$classes.qF;
				var page = '.' + $mdgriffith$elm_ui$Internal$Style$classes.D;
				var left = '.' + $mdgriffith$elm_ui$Internal$Style$classes.ox;
				var halfY = $elm$core$String$fromFloat(y / 2) + 'px';
				var halfX = $elm$core$String$fromFloat(x / 2) + 'px';
				var column = '.' + $mdgriffith$elm_ui$Internal$Style$classes.dH;
				var _class = '.' + cls;
				var any = '.' + $mdgriffith$elm_ui$Internal$Style$classes.sF;
				return $elm$core$List$concat(
					_List_fromArray(
						[
							A4(
			...
			case 9:
				...
				var msPosition = A2(
					...
					_List_fromArray(
						[
							'-ms-grid-row: ' + ($elm$core$String$fromInt(position.re) + ';'),
							'-ms-grid-row-span: ' + ($elm$core$String$fromInt(position.iP) + ';'),
							'-ms-grid-column: ' + ($elm$core$String$fromInt(position.o_) + ';'),
				...
				var modernPosition = A2(
					...
					_List_fromArray(
						[
							'grid-row: ' + ($elm$core$String$fromInt(position.re) + (' / ' + ($elm$core$String$fromInt(position.re + position.iP) + ';'))),
							'grid-column: ' + ($elm$core$String$fromInt(position.o_) + (' / ' + ($elm$core$String$fromInt(position.o_ + position.$7) + ';')))
						]));
				var _class = '.grid-pos-' + ($elm$core$String$fromInt(position.re) + ('-' + ($elm$core$String$fromInt(position.o_) + ('-' + ($elm$core$String$fromInt(position.$7) + ('-' + $elm$core$String$fromInt(position.iP)))))));
	...
	function (fontToAdjust, _v0, otherFontName) {
		...
		return A2(
			...
			_Utils_ap(
				...
				A3($mdgriffith$elm_ui$Internal$Model$fontRule, name, $mdgriffith$elm_ui$Internal$Style$classes.tW, full)));
	});
...
var $mdgriffith$elm_ui$Internal$Model$adjust = F3(
	function (size, height, vertical) {
		return {iP: height / size, vV: size, sa: vertical};
	});
...
var $mdgriffith$elm_ui$Internal$Model$convertAdjustment = function (adjustment) {
	var lines = _List_fromArray(
		[adjustment.s$, adjustment.sO, adjustment.tx, adjustment.uF]);
	var lineHeight = 1.5;
	...
	return {
		s$: A3($mdgriffith$elm_ui$Internal$Model$adjust, capitalSize, ascender - newBaseline, capitalVertical),
		pu: A3($mdgriffith$elm_ui$Internal$Model$adjust, fullSize, ascender - descender, fullVertical)
	};
};
	...
	return _Utils_Tuple2(
		...
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'line-height',
				$elm$core$String$fromFloat(converted.iP)),
				_Utils_Tuple2(
				'vertical-align',
				$elm$core$String$fromFloat(converted.sa) + 'em'),
				_Utils_Tuple2(
				'font-size',
				...
				return {
					...
					l_: function () {
						...
						if (_v1.$ === 1) {
							...
							var topLevel = _v1.a;
							return A2($elm$core$List$cons, topLevel, rendered.l_);
						}
					}()
				};
			});
	...
	function (_static, opts, styles, children) {
		var dynamicStyleSheet = A2(
			...
			A3(
				...
				_Utils_Tuple2(
					$elm$core$Set$empty,
					$mdgriffith$elm_ui$Internal$Model$renderFocusStyle(opts.tR)),
				styles).b);
		return _static ? A2(
			$elm$core$List$cons,
			_Utils_Tuple2(
					...
					return A3(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.vU))
									]))
							]));
			}
		}();
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.sF, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.mW, $mdgriffith$elm_ui$Internal$Style$classes.dI, $mdgriffith$elm_ui$Internal$Style$classes.sB])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.sF, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.mW, $mdgriffith$elm_ui$Internal$Style$classes.dI, $mdgriffith$elm_ui$Internal$Style$classes.sz])))
						]),
					_List_fromArray(
						[html])) : html));
			case 1:
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.sF, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.mW, $mdgriffith$elm_ui$Internal$Style$classes.sA])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.sF, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.mW, $mdgriffith$elm_ui$Internal$Style$classes.sy])))
						]),
					_List_fromArray(
						[html])) : html));
			default:
...
var $mdgriffith$elm_ui$Internal$Model$textElementClasses = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.dE + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.og + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.nO)))));
...
var $mdgriffith$elm_ui$Internal$Model$textElementFillClasses = $mdgriffith$elm_ui$Internal$Style$classes.sF + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.dE + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.oh + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.nP)))));
...
var $mdgriffith$elm_ui$Internal$Model$createElement = F3(
	function (context, children, rendered) {
					...
					case 1:
						...
						return _Utils_eq(context, $mdgriffith$elm_ui$Internal$Model$asParagraph) ? _Utils_Tuple2(
							A2(
								...
								_Utils_Tuple2(
									key,
									A2(styled.ud, $mdgriffith$elm_ui$Internal$Model$NoStyleSheet, context)),
								htmls),
							$elm$core$List$isEmpty(existingStyles) ? styled.rL : _Utils_ap(styled.rL, existingStyles)) : _Utils_Tuple2(
							A2(
								$elm$core$List$cons,
								_Utils_Tuple2(
									key,
									A2(styled.ud, $mdgriffith$elm_ui$Internal$Model$NoStyleSheet, context)),
								htmls),
							$elm$core$List$isEmpty(existingStyles) ? styled.rL : _Utils_ap(styled.rL, existingStyles));
					case 2:
						var str = child.a;
						return _Utils_Tuple2(
							A2(
		...
		if (children.$ === 1) {
			...
			var keyed = _v1.a;
			var styles = _v1.b;
			var newStyles = $elm$core$List$isEmpty(styles) ? rendered.rL : _Utils_ap(rendered.rL, styles);
			if (!newStyles.b) {
				return $mdgriffith$elm_ui$Internal$Model$Unstyled(
					A5(
						...
						$mdgriffith$elm_ui$Internal$Model$Keyed(
							A3($mdgriffith$elm_ui$Internal$Model$addKeyedChildren, 'nearby-element-pls', keyed, rendered.s4)),
				...
				return $mdgriffith$elm_ui$Internal$Model$Unstyled(
					A5(
						...
						$mdgriffith$elm_ui$Internal$Model$Unkeyed(
							A2($mdgriffith$elm_ui$Internal$Model$addChildren, unkeyed, rendered.s4)),
						$mdgriffith$elm_ui$Internal$Model$NoStyleSheet));
			} else {
				var allStyles = newStyles;
				return $mdgriffith$elm_ui$Internal$Model$Styled(
						...
						switch (location) {
							case 0:
								return A2(
									...
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fT, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.sp]));
							case 1:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fT, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.sQ]));
							case 2:
								return A2(
									$elm$core$String$join,
									' ',
							...
							case 3:
								return A2(
									...
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fT, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.u0]));
							case 4:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.fT, $mdgriffith$elm_ui$Internal$Style$classes.vU, $mdgriffith$elm_ui$Internal$Style$classes.uj]));
							default:
								return A2(
									$elm$core$String$join,
									' ',
			...
			_List_fromArray(
				[
					...
					switch (elem.$) {
						...
						default:
							var styled = elem.a;
							return A2(styled.ud, $mdgriffith$elm_ui$Internal$Model$NoStyleSheet, $mdgriffith$elm_ui$Internal$Model$asEl);
					}
				}()
				]));
	});
...
var $mdgriffith$elm_ui$Internal$Model$alignXName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.nx + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.ox);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.nx + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.oy);
		default:
			return $mdgriffith$elm_ui$Internal$Style$classes.nx + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.sw);
	}
};
var $mdgriffith$elm_ui$Internal$Model$alignYName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.ny + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.sC);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.ny + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.sv);
		default:
			return $mdgriffith$elm_ui$Internal$Style$classes.ny + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.sx);
	}
};
	...
	switch (h.$) {
		case 0:
			...
			return _Utils_Tuple3(
				...
				$mdgriffith$elm_ui$Internal$Style$classes.pI + (' ' + name),
				_List_fromArray(
					[
		...
		case 1:
			return _Utils_Tuple3(
				...
				$mdgriffith$elm_ui$Internal$Style$classes.nO,
				_List_Nil);
		case 2:
			var portion = h.a;
			return (portion === 1) ? _Utils_Tuple3(
				A2($mdgriffith$elm_ui$Internal$Flag$add, $mdgriffith$elm_ui$Internal$Flag$heightFill, $mdgriffith$elm_ui$Internal$Flag$none),
				$mdgriffith$elm_ui$Internal$Style$classes.nP,
				_List_Nil) : _Utils_Tuple3(
				A2($mdgriffith$elm_ui$Internal$Flag$add, $mdgriffith$elm_ui$Internal$Flag$heightFill, $mdgriffith$elm_ui$Internal$Flag$none),
				$mdgriffith$elm_ui$Internal$Style$classes.pJ + (' height-fill-' + $elm$core$String$fromInt(portion)),
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.sF + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.dH + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
...
var $mdgriffith$elm_ui$Internal$Model$renderWidth = function (w) {
	switch (w.$) {
		case 0:
			...
			return _Utils_Tuple3(
				$mdgriffith$elm_ui$Internal$Flag$none,
				$mdgriffith$elm_ui$Internal$Style$classes.sj + (' width-px-' + $elm$core$String$fromInt(px)),
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
		...
		case 1:
			return _Utils_Tuple3(
				...
				$mdgriffith$elm_ui$Internal$Style$classes.og,
				_List_Nil);
		case 2:
			var portion = w.a;
			return (portion === 1) ? _Utils_Tuple3(
				A2($mdgriffith$elm_ui$Internal$Flag$add, $mdgriffith$elm_ui$Internal$Flag$widthFill, $mdgriffith$elm_ui$Internal$Flag$none),
				$mdgriffith$elm_ui$Internal$Style$classes.oh,
				_List_Nil) : _Utils_Tuple3(
				A2($mdgriffith$elm_ui$Internal$Flag$add, $mdgriffith$elm_ui$Internal$Flag$widthFill, $mdgriffith$elm_ui$Internal$Flag$none),
				$mdgriffith$elm_ui$Internal$Style$classes.sk + (' width-fill-' + $elm$core$String$fromInt(portion)),
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.sF + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.re + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
					...
					case 7:
						...
						if (A2($mdgriffith$elm_ui$Internal$Flag$present, $mdgriffith$elm_ui$Internal$Flag$width, has)) {
							...
							switch (width.$) {
								case 0:
									var px = width.a;
									var $temp$classes = ($mdgriffith$elm_ui$Internal$Style$classes.sj + (' width-px-' + $elm$core$String$fromInt(px))) + (' ' + classes),
										$temp$node = node,
									...
									continue gatherAttrRecursive;
								case 1:
									var $temp$classes = classes + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.og),
										$temp$node = node,
										$temp$has = A2(
								...
								case 2:
									var portion = width.a;
									if (portion === 1) {
										var $temp$classes = classes + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.oh),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
										...
										var $temp$classes = classes + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.sk + (' width-fill-' + $elm$core$String$fromInt(portion)))),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
											$mdgriffith$elm_ui$Internal$Flag$widthFill,
						...
						if (A2($mdgriffith$elm_ui$Internal$Flag$present, $mdgriffith$elm_ui$Internal$Flag$height, has)) {
							...
							switch (height.$) {
								case 0:
									...
									var name = 'height-px-' + val;
									var $temp$classes = $mdgriffith$elm_ui$Internal$Style$classes.pI + (' ' + (name + (' ' + classes))),
										$temp$node = node,
									...
									continue gatherAttrRecursive;
								case 1:
									var $temp$classes = $mdgriffith$elm_ui$Internal$Style$classes.nO + (' ' + classes),
										$temp$node = node,
										$temp$has = A2(
										$mdgriffith$elm_ui$Internal$Flag$add,
								...
								case 2:
									...
									if (portion === 1) {
										var $temp$classes = $mdgriffith$elm_ui$Internal$Style$classes.nP + (' ' + classes),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
										...
										var $temp$classes = classes + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.pJ + (' height-fill-' + $elm$core$String$fromInt(portion)))),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
											$mdgriffith$elm_ui$Internal$Flag$heightFill,
					...
					case 9:
						...
						var newStyles = function () {
							switch (elem.$) {
								...
								default:
									var styled = elem.a;
									return _Utils_ap(styles, styled.rL);
							}
						}();
						var $temp$classes = classes,
							$temp$node = node,
...
var $mdgriffith$elm_ui$Element$column = F2(
	function (attrs, children) {
		return A4(
			...
			A2(
				$elm$core$List$cons,
				$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.tj + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.h3)),
				A2(
					$elm$core$List$cons,
...
var $mdgriffith$elm_ui$Element$Font$bold = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$fontWeight, $mdgriffith$elm_ui$Internal$Style$classes.sU);
...
var $mdgriffith$elm_ui$Element$pointer = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$cursor, $mdgriffith$elm_ui$Internal$Style$classes.tm);
		...
		return A4(
			...
			A2(
				...
				A2(
					...
					A2(
						$elm$core$List$cons,
						$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.mY + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.dI + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.vL + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.qu)))))),
						A2(
							$elm$core$List$cons,
...
var $mdgriffith$elm_ui$Internal$Model$boxShadowClass = function (shadow) {
	return $elm$core$String$concat(
		_List_fromArray(
			[
				shadow.pT ? 'box-inset' : 'box-',
				$mdgriffith$elm_ui$Internal$Model$floatClass(shadow.j5.a) + 'px',
				$mdgriffith$elm_ui$Internal$Model$floatClass(shadow.j5.b) + 'px',
				$mdgriffith$elm_ui$Internal$Model$floatClass(shadow.sS) + 'px',
				$mdgriffith$elm_ui$Internal$Model$floatClass(shadow.vV) + 'px',
				$mdgriffith$elm_ui$Internal$Model$formatColorClass(shadow.tb)
			]));
};
var $mdgriffith$elm_ui$Internal$Flag$shadows = $mdgriffith$elm_ui$Internal$Flag$flag(19);
var $mdgriffith$elm_ui$Element$Border$shadow = function (almostShade) {
	var shade = {sS: almostShade.sS, tb: almostShade.tb, pT: false, j5: almostShade.j5, vV: almostShade.vV};
	return A2(
		$mdgriffith$elm_ui$Internal$Model$StyleClass,
	...
	_Utils_ap(
		...
		_List_fromArray(
			[
				...
				{
					sS: 10,
					tb: A4($mdgriffith$elm_ui$Element$rgba, 0, 0, 0, 0.05),
					j5: _Utils_Tuple2(0, 2),
					vV: 1
				}),
...
var $Orasund$elm_ui_framework$Framework$layoutOptions = $elm$core$List$singleton(
	$mdgriffith$elm_ui$Element$focusStyle(
		{
			sJ: $elm$core$Maybe$Nothing,
			sV: $elm$core$Maybe$Just($Orasund$elm_ui_framework$Framework$Color$turquoise),
			vS: $elm$core$Maybe$Just(
				{
					sS: 10,
					tb: $Orasund$elm_ui_framework$Framework$Color$turquoise,
					j5: _Utils_Tuple2(0, 0),
					vV: 1
				})
		}));
...
var $mdgriffith$elm_ui$Internal$Model$focusDefaultStyle = {
	sJ: $elm$core$Maybe$Nothing,
	sV: $elm$core$Maybe$Nothing,
	vS: $elm$core$Maybe$Just(
		{
			sS: 0,
			tb: A4($mdgriffith$elm_ui$Internal$Model$Rgba, 155 / 255, 203 / 255, 1, 1),
			j5: _Utils_Tuple2(0, 0),
			vV: 3
		})
};
var $mdgriffith$elm_ui$Internal$Model$optionsToRecord = function (options) {
				...
				case 0:
					...
					if (_v4.$ === 1) {
						return _Utils_update(
							record,
							{
								ub: $elm$core$Maybe$Just(hoverable)
							});
					} else {
						return record;
					}
				case 1:
					...
					if (_v5.$ === 1) {
						return _Utils_update(
							record,
							{
								tR: $elm$core$Maybe$Just(focusStyle)
							});
					} else {
						return record;
					}
				default:
					...
					if (_v6.$ === 1) {
						return _Utils_update(
							record,
							{
								uN: $elm$core$Maybe$Just(renderMode)
							});
					} else {
						return record;
					}
	...
	return andFinally(
		A3(
			$elm$core$List$foldr,
			combine,
			{tR: $elm$core$Maybe$Nothing, ub: $elm$core$Maybe$Nothing, uN: $elm$core$Maybe$Nothing},
			options));
};
			...
			A2(
				...
				$mdgriffith$elm_ui$Internal$Model$htmlClass(
					A2(
						...
						' ',
						_List_fromArray(
							[$mdgriffith$elm_ui$Internal$Style$classes.rc, $mdgriffith$elm_ui$Internal$Style$classes.sF, $mdgriffith$elm_ui$Internal$Style$classes.vU]))),
...
var $Orasund$elm_ui_framework$Framework$layout = function (attributes) {
	return A2(
		...
		{u8: $Orasund$elm_ui_framework$Framework$layoutOptions},
				...
				function (currentMax, remainingList) {
					...
					while (true) {
						if (!remainingList.b) {
							...
							if (_Utils_cmp(next.bE, currentMax.bE) > 0) {
...
var $author$project$SR$Elements$spectatorSelectedRankingHeaderEl = F2(
	function (userInfo, r) {
		return A2(
			$mdgriffith$elm_ui$Element$el,
			$Orasund$elm_ui_framework$Framework$Heading$h5,
			$mdgriffith$elm_ui$Element$text(userInfo.dx + (' you\'re interested in joining' + (' - ' + (r.fR + (' Id no: ' + (r.z + (' . \n Which is owned by ' + (r.n1 + (' id no: ' + r.f$))))))))));
	});
	...
	function (attrs, children) {
		...
		if (spaced.$ === 1) {
			return A4(
				...
				A2(
					$elm$core$List$cons,
					$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.h3 + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.dI + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.oi)))),
					A2(
						$elm$core$List$cons,
						$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$shrink),
						A2(
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$el,
						$Orasund$elm_ui_framework$Framework$Heading$h5,
						$mdgriffith$elm_ui$Element$text(userInfo.dx + (' - Are you sure you want to join' + (ranking.n1 + '\'s  ranking?')))),
						A2(
						$mdgriffith$elm_ui$Element$column,
						A2(
							$elm$core$List$cons,
							...
							[
								...
								A3(
									...
									A2(
										$elm$core$Maybe$withDefault,
										$author$project$Data$Ranking$emptyRank,
										$author$project$Data$Ranking$gotLowestRank(ranking.m8)).bE)),
...
var $author$project$SR$Elements$memberSelectedRankingHeaderEl = F2(
	function (userInfo, r) {
		return A2(
			$mdgriffith$elm_ui$Element$el,
			$Orasund$elm_ui_framework$Framework$Heading$h5,
			$mdgriffith$elm_ui$Element$text(userInfo.dx + (' you\'re a member of' + (' - ' + (r.fR + (' Id no: ' + (r.z + (' . \n Which is owned by ' + (r.n1 + (' id no: ' + r.f$))))))))));
	});
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$el,
						$Orasund$elm_ui_framework$Framework$Heading$h5,
						$mdgriffith$elm_ui$Element$text(userInfo.dx + (' - Are you sure you want to leave ' + (ranking.n1 + '\'s  ranking?')))),
						A2(
						$mdgriffith$elm_ui$Element$column,
						A2(
							$elm$core$List$cons,
						...
						_List_fromArray(
							[
								...
								$author$project$Pages$Hardware$infoBtn,
								'Leave',
								A2($author$project$Pages$Hardware$ConfirmLeaveMemberRanking, ranking, userInfo.r6)),
								A2($author$project$Pages$Hardware$infoBtn, 'Cancel', $author$project$Pages$Hardware$CancelFetchedMember)
							]))
					])));
	});
		...
		var userRank = A2(
			...
			$elm$core$List$head(
				A2(
					$elm$core$List$filter,
					function (r) {
						return _Utils_eq(r.aC.z, userInfo.r6);
					},
					ranking.m8)));
		var otherRank = A2(
			$elm$core$Maybe$map,
...
var $Orasund$elm_ui_framework$Framework$Grid$section = _Utils_ap(
	...
	_List_fromArray(
		[
			...
			$mdgriffith$elm_ui$Element$Border$color($Orasund$elm_ui_framework$Framework$Color$lightGrey),
			$mdgriffith$elm_ui$Element$paddingEach(
			{oN: 30, p5: 0, rb: 0, rR: 10})
		]));
var $author$project$Pages$Hardware$createChallengeView = F3(
	function (uinfo, rank, ranking) {
		return (!_Utils_eq(rank.aC.z, uinfo.r6)) ? ((!A2($author$project$Pages$Hardware$isUserRankedHigher, uinfo, ranking)) ? A2(
			$Orasund$elm_ui_framework$Framework$responsiveLayout,
			_List_Nil,
			A2(
				$mdgriffith$elm_ui$Element$column,
					...
					[
						...
						_List_fromArray(
							[
								...
								$mdgriffith$elm_ui$Element$el,
								_List_Nil,
								$mdgriffith$elm_ui$Element$text(uinfo.dx + (' you are challenging ' + rank.aC.dx)))
							])),
						A2(
						$mdgriffith$elm_ui$Element$el,
						_List_Nil,
								...
								_List_fromArray(
									[
										...
										_Utils_ap($Orasund$elm_ui_framework$Framework$Button$simple, $Orasund$elm_ui_framework$Framework$Color$simple),
										{
											...
											ar: $elm$core$Maybe$Just($author$project$Pages$Hardware$Cancel)
										}),
										A2(
										$mdgriffith$elm_ui$Element$Input$button,
					...
					[
						...
						_List_fromArray(
							[
								...
								$mdgriffith$elm_ui$Element$el,
								_List_Nil,
								$mdgriffith$elm_ui$Element$text(rank.aC.dx + ' you can\'t challenge yourself! '))
							])),
						A2(
						$mdgriffith$elm_ui$Element$column,
						_Utils_ap($Orasund$elm_ui_framework$Framework$Card$simple, $Orasund$elm_ui_framework$Framework$Grid$simple),
...
var $author$project$SR$Elements$colors = {
	oK: A3($mdgriffith$elm_ui$Element$rgb255, 0, 0, 0),
	mO: A3($mdgriffith$elm_ui$Element$rgb255, 2, 7, 239),
	o0: A3($mdgriffith$elm_ui$Element$rgb255, 204, 75, 75),
	o5: A3($mdgriffith$elm_ui$Element$rgb, 0, 0, 0.9),
	pE: A3($mdgriffith$elm_ui$Element$rgb255, 0, 153, 0),
	pF: A3($mdgriffith$elm_ui$Element$rgb, 0.9, 0.9, 0.9),
	p6: A3($mdgriffith$elm_ui$Element$rgb255, 0, 128, 255),
	q_: A3($mdgriffith$elm_ui$Element$rgb255, 102, 0, 102),
	q$: A3($mdgriffith$elm_ui$Element$rgb, 0.8, 0, 0),
	sh: A3($mdgriffith$elm_ui$Element$rgb255, 255, 255, 255)
};
...
var $author$project$SR$Elements$ladderCityValidation = function (ranking) {
	return $author$project$Utils$Validation$Validate$isValid4to20Chars(ranking.dG.h$) ? A2(
		$mdgriffith$elm_ui$Element$el,
		_Utils_ap(
			A2(
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Font$color($author$project$SR$Elements$colors.pE),
		...
		_Utils_ap(
			A2(
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Font$color($author$project$SR$Elements$colors.mO),
						$mdgriffith$elm_ui$Element$Font$alignLeft
					])),
			_List_fromArray(
				[
	...
	return $author$project$Utils$Validation$Validate$isValid4to20Chars(ranking.fR) ? A2(
		...
		_Utils_ap(
			A2(
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Font$color($author$project$SR$Elements$colors.q$),
						$mdgriffith$elm_ui$Element$Font$alignLeft
					])),
			_List_fromArray(
				[
...
var $author$project$SR$Elements$ladderStreetValidation = function (ranking) {
	return $author$project$Utils$Validation$Validate$isValid4to20Chars(ranking.dG.lF) ? A2(
		$mdgriffith$elm_ui$Element$el,
		_Utils_ap(
			A2(
				$elm$core$List$append,
...
var $author$project$Pages$Hardware$isValidatedForAllLadderDetailsInput = function (ranking) {
	return $author$project$Utils$Validation$Validate$isValid4to20Chars(ranking.fR) && (((ranking.dG.lF === '') || $author$project$Utils$Validation$Validate$isValid4to20Chars(ranking.dG.lF)) && ((ranking.dG.h$ === '') || $author$project$Utils$Validation$Validate$isValid4to20Chars(ranking.dG.h$)));
};
...
var $author$project$Pages$Hardware$rankingDetailsConfirmPanel = F2(
	function (ranking, userInfo) {
		return $author$project$Pages$Hardware$isValidatedForAllLadderDetailsInput(ranking) ? A2(
			...
			_List_fromArray(
				[
					((ranking.dG.lF === '') && (ranking.dG.h$ === '')) ? $author$project$SR$Elements$warningParagraph : $mdgriffith$elm_ui$Element$text('Click to continue ...'),
					A2(
					$mdgriffith$elm_ui$Element$column,
					_Utils_ap($Orasund$elm_ui_framework$Framework$Card$simple, $Orasund$elm_ui_framework$Framework$Grid$simple),
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									...
									{
										I: $mdgriffith$elm_ui$Element$text('Cancel'),
										ar: $elm$core$Maybe$Just($author$project$Pages$Hardware$CancelCreateNewRanking)
									}),
									A2(
									$mdgriffith$elm_ui$Element$Input$button,
									_Utils_ap(
			...
			switch (position) {
				case 2:
					return A4(
						...
						A2(
							$elm$core$List$cons,
							$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.m6),
							attrs),
						$mdgriffith$elm_ui$Internal$Model$Unkeyed(
							_List_fromArray(
								[labelElement, input])));
...
var $mdgriffith$elm_ui$Element$clip = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$overflow, $mdgriffith$elm_ui$Internal$Style$classes.s7);
...
var $mdgriffith$elm_ui$Element$Input$negateBox = function (box) {
	return {oN: -box.oN, p5: -box.p5, rb: -box.rb, rR: -box.rR};
};
						...
						if (isMultiline) {
							...
							return _Utils_update(
								els,
								{
									...
									x: A2($elm$core$List$cons, reducedVerticalPadding, els.x)
								});
						}
					case 6:
						var _v3 = attr.b;
	...
	function (isMultiline, stacked, attrs) {
		return function (redist) {
			...
			A3(
				...
				{bn: _List_Nil, _: _List_Nil, aA: _List_Nil, x: _List_Nil, g$: _List_Nil},
				attrs));
	});
				...
				_Utils_ap(
					_List_fromArray(
						[
							...
							$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.qu + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.vc)),
...
var $mdgriffith$elm_ui$Element$Input$textHelper = F3(
	function (textInput, attrs, textOptions) {
					...
					if (!_v4.$) {
						...
						return _List_fromArray(
							[
								$mdgriffith$elm_ui$Element$clip,
								$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill),
								$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.un),
				...
				_Utils_ap(
					_List_fromArray(
						[
							...
							$mdgriffith$elm_ui$Element$Input$spellcheck(textInput.c7),
							A2(
							$elm$core$Maybe$withDefault,
					...
					_Utils_ap(
						(heightConstrained ? $elm$core$List$cons($mdgriffith$elm_ui$Element$scrollbarY) : $elm$core$Basics$identity)(
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
									A2($elm$core$List$any, $mdgriffith$elm_ui$Element$Input$hasFocusStyle, withDefaults) ? $mdgriffith$elm_ui$Internal$Model$NoAttribute : $mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.ps),
								...
								A2(
									...
									A2(
										...
										A2(
											...
											A2(
												$elm$core$List$cons,
												$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.up),
												redistributed.g$)))),
								$mdgriffith$elm_ui$Internal$Model$Unkeyed(
									function () {
										if (textOptions.dE === '') {
											...
											return _List_fromArray(
												[
													...
													A2(
														...
														_List_fromArray(
															[
																$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.uo)
															]),
														_List_fromArray(
															[
										...
										if (_v2.$ === 1) {
											...
											return _List_fromArray(
												[
													...
													A3($mdgriffith$elm_ui$Element$Input$renderPlaceholder, place, redistributed.bn, textOptions.dE === ''))
												]);
										}
									}()
									])))),
		...
		return A3(
			...
			A2(
				$elm$core$List$cons,
				A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$cursor, $mdgriffith$elm_ui$Internal$Style$classes.tn),
				A2(
					$elm$core$List$cons,
...
var $author$project$SR$Elements$ownedSelectedRankingHeaderEl = function (r) {
	return A2(
		$mdgriffith$elm_ui$Element$el,
		$Orasund$elm_ui_framework$Framework$Heading$h5,
		$mdgriffith$elm_ui$Element$text(r.n1 + (' this your owned ranking' + (' - ' + (r.fR + (' . \n Id is: ' + r.z))))));
};
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$el,
						$Orasund$elm_ui_framework$Framework$Heading$h5,
						$mdgriffith$elm_ui$Element$text(ranking.n1 + (' - Are you sure you want to challenge ' + (rank.aC.dx + '?')))),
						A2(
						$mdgriffith$elm_ui$Element$wrappedRow,
						_List_Nil,
						_List_fromArray(
...
var $author$project$Data$Ranking$isUserOwnerOfRankning = F2(
	function (userid, ranking) {
		return _Utils_eq(ranking.f$, userid) ? true : false;
	});
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$el,
						$Orasund$elm_ui_framework$Framework$Heading$h5,
						$mdgriffith$elm_ui$Element$text(uinfo.dx + (' - Are you ready to enter your result vs ' + (rank.ao.dx + '?')))),
						A2($author$project$Data$Ranking$isUserOwnerOfRankning, uinfo.r6, ranking) ? A2(
						$mdgriffith$elm_ui$Element$wrappedRow,
						_List_Nil,
						_List_fromArray(
							[
...
var $author$project$Pages$Hardware$ClickedLedgerConnect = {$: 23};
		...
		A2(
			...
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$text('\n'),
					A2($author$project$Pages$Hardware$infoBtn, 'Connect Wallet', $author$project$Pages$Hardware$ClickedLedgerConnect),
					$mdgriffith$elm_ui$Element$text('\n'),
					A2(
					$mdgriffith$elm_ui$Element$el,
					$Orasund$elm_ui_framework$Framework$Heading$h6,
					$mdgriffith$elm_ui$Element$text(
						model.nS ? 'Connected' : 'Not connected yet')),
					function () {
					var _v0 = model.dJ;
					if (!_v0.b) {
			...
			[
					...
					$author$project$Pages$Hardware$enableButton(false)),
				{
					I: $mdgriffith$elm_ui$Element$text(
						$elm$core$String$fromInt(r.bE) + ('. ' + (r.aC.dx + (' vs ' + r.ao.dx)))),
					ar: $elm$core$Maybe$Just(
...
var $author$project$Pages$Hardware$determineButtonType = F2(
	function (loggedInUsersRank, rankBeingIterated) {
		var rankBeingIteratedIsInAChallenge = !_Utils_eq(rankBeingIterated.ao.z, $author$project$Extras$Constants$noCurrentChallengerId);
		var loggedInUserIsInAChallenge = !_Utils_eq(loggedInUsersRank.ao.z, $author$project$Extras$Constants$noCurrentChallengerId);
		return loggedInUserIsInAChallenge ? (_Utils_eq(loggedInUsersRank.aC.z, rankBeingIterated.aC.z) ? 1 : (_Utils_eq(loggedInUsersRank.aC.z, rankBeingIterated.ao.z) ? 2 : 4)) : (rankBeingIteratedIsInAChallenge ? (_Utils_eq(loggedInUsersRank.aC.z, rankBeingIterated.aC.z) ? 1 : 2) : (_Utils_eq(loggedInUsersRank.bE - 1, rankBeingIterated.bE) ? 3 : 4));
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				{
					I: $mdgriffith$elm_ui$Element$text(
						$elm$core$String$fromInt(r.bE) + ('. ' + r.aC.dx)),
					ar: $elm$core$Maybe$Just(
						$author$project$Pages$Hardware$ViewRank(r))
				})
			]));
	...
	function (userid, ladder) {
		return $elm$core$List$head(
			A2(
				$elm$core$List$filter,
				function (r) {
					return _Utils_eq(r.aC.z, userid);
				},
				ladder));
	});
var $author$project$Pages$Hardware$playerbuttons = F2(
	function (r, u) {
		var currentUserRank = A2(
			$elm$core$Maybe$withDefault,
			$author$project$Data$Ranking$emptyRank,
			A2($author$project$Pages$Hardware$findUserRank, u.r6, r.m8));
		return A2(
			$mdgriffith$elm_ui$Element$column,
			$Orasund$elm_ui_framework$Framework$Grid$section,
			_List_fromArray(
...
var $mdgriffith$elm_ui$Element$Input$newPassword = F2(
	function (attrs, pass) {
		return A3(
			...
			{
				bR: $elm$core$Maybe$Just('new-password'),
				c7: false,
...
var $author$project$SR$Elements$passwordValidView = function (userInfo) {
	return $author$project$Utils$Validation$Validate$isValid4to8Chars(userInfo.n2) ? A2(
		$mdgriffith$elm_ui$Element$el,
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$Font$color($author$project$SR$Elements$colors.pE),
...
var $author$project$Pages$Hardware$isValidatedForAllUserDetailsInput = function (userInfo) {
	return ($author$project$Utils$Validation$Validate$isEmailValid(
		A2($elm$core$Maybe$withDefault, '', userInfo.nI)) && ($author$project$Utils$Validation$Validate$isValid4to8Chars(userInfo.dx) && ($author$project$Utils$Validation$Validate$is20CharMax('') && ($author$project$Utils$Validation$Validate$isEmailValid(
		A2($elm$core$Maybe$withDefault, '', userInfo.nI)) && $author$project$Utils$Validation$Validate$isMobileValid(
		A2($elm$core$Maybe$withDefault, '', userInfo.nX)))))) ? true : false;
};
var $author$project$Pages$Hardware$userDetailsConfirmPanel = function (userInfo) {
	return (($author$project$Utils$Validation$Validate$isEmailValid(
		A2($elm$core$Maybe$withDefault, '', userInfo.nI)) && ($elm$core$String$length(
		A2($elm$core$Maybe$withDefault, '', userInfo.nI)) > 0)) && (($author$project$Utils$Validation$Validate$isValid4to8Chars(userInfo.n2) && ($elm$core$String$length(userInfo.n2) > 0)) && ((userInfo.dx === '') || $author$project$Utils$Validation$Validate$isValid4to8Chars(userInfo.dx)))) ? A2(
		$mdgriffith$elm_ui$Element$column,
		$Orasund$elm_ui_framework$Framework$Grid$section,
		_List_fromArray(
			[
					...
					[
						...
						_List_fromArray(
							[
								...
								{
									I: $mdgriffith$elm_ui$Element$text('Cancel'),
									ar: $elm$core$Maybe$Just($author$project$Pages$Hardware$CancelRegistration)
								}),
								A2(
								$mdgriffith$elm_ui$Element$Input$button,
								_Utils_ap(
								...
								[
											...
											])),
									{
										...
										dz: $elm$core$Maybe$Just(
											A2(
												...
												_List_Nil,
												$mdgriffith$elm_ui$Element$text('Email'))),
										dE: A2($elm$core$Maybe$withDefault, '', userInfo.nI)
									}),
									$author$project$Pages$Hardware$emailValidationErr(
									A2($elm$core$Maybe$withDefault, '', userInfo.nI)),
									A2(
											...
											])),
									{
										...
										dE: A2(
											...
											A2($elm$core$Maybe$withDefault, '', userInfo.nX),
											25)
									}),
											...
											switch (_v1.$) {
												...
												case 12:
													return A3(
														...
														$author$project$Data$User$gotUserInfo(model.aD),
														model.n8,
														$author$project$Data$Ranking$gotRanking(model.aj));
												default:
													return A2(
...
var $author$project$Main$view = function (model) {
	...
	return {
		sT: _List_fromArray(
			[
				$author$project$Main$pageHeader(model.D),
				$author$project$Main$showVideoOrBanner(model.D),
				contentByPage,
...
var $author$project$Main$main = $elm$browser$Browser$application(
	{uk: $author$project$Main$init, u2: $author$project$Main$ChangedUrl, u3: $author$project$Main$ClickedLink, v6: $author$project$Main$subscriptions, wx: $author$project$Main$update, wA: $author$project$Main$view});
