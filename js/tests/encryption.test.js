...
describe("Web Crypto API - AES Encryption", () => {
    ...
    it("should encrypt and decrypt the message correctly", async () => {
        ...
        const encryptedReadyForStorage = await encrypt( JSON.stringify(elmENCRYPTMessageAsJson), elmENCRYPTMessageAsJson.password);

        expect(encryptedReadyForStorage).not.toBeNull();
        expect(encryptedReadyForStorage).not.toBeUndefined();
        const decrypted = await decrypt(encryptedReadyForStorage, elmENCRYPTMessageAsJson.password);
    ...
    it("should generate different encrypted outputs for the same message", async () => {
        const firstEncryption = await encrypt(elmENCRYPTMessageAsJson, elmENCRYPTMessageAsJson.password);
        const secondEncryption = await encrypt(elmENCRYPTMessageAsJson, elmENCRYPTMessageAsJson.password);
    ...
    it("should store encrypted data with the expected structure", async () => {
        // NOTE: We stringify the message before passing it to ENCRYPT (not other parts of code)
        const encryptedinStorage = await encrypt(JSON.stringify(elmENCRYPTMessageAsJson), elmENCRYPTMessageAsJson.password);

        expect(encryptedinStorage).not.toBeNull();
        expect(encryptedinStorage).not.toBeUndefined();
        const decrypted = await decrypt(encryptedinStorage, elmENCRYPTMessageAsJson.password);
        expect(decrypted).not.toBeNull();
        expect(decrypted).not.toBeUndefined();
        const parsedDecrypted = JSON.parse(decrypted);
